"""
41.crate_scan_setup_random_obstacles.py
39.crate_scan_setup_random_layout.py의 복사본 - 35.py/38.py/39.py는 그대로 두고
(전부 잘 동작하는 버전이라 보존), 여기서는 39.py의 테이블 박스 위치 랜덤화에
더해 크레이트 안 더미 장애물 박스 2개(DummyA/B)의 크기와 위치도 매 실행마다
무작위로 흔든다 - trunk_map.json에 그 결과(장애물 좌표/크기)가 매번 정확히
반영되는지 보기 위함.
...
#   HEADLESS=1 isaac_python 41.crate_scan_setup_random_obstacles.py
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"41.crate_scan_setup_random_obstacles.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_41scan_random_obstacles"
...
print("\n[완료] 41.crate_scan_setup_random_obstacles.py 끝.\n", flush=True)
