"""
12.trunk_scan_hidden_gripper.py 기반 + 트렁크 안쪽 깊은 벽(뒤쪽)에 붙인 정적 테스트 박스 추가.

목적: 13.export_trunk_map.py 파이프라인이 트렁크 내부의 임의 장애물(박스)을 실제로
결과물(trunk_map.json)에 반영하는지 검증하는 테스트 run.

추가된 것:
- TEST_BOX_SIZE/TEST_BOX_GAP: 깊은 벽(x_max)에 거의 붙여서(GAP만큼 띄움), y 중앙, 바닥에
  놓는 정적(물리 없음, cart/vehicle과 동일하게 CollisionAPI만 적용) 박스 하나.
- add_static_test_box(): 씬 구성 단계(차량 로드 직후)에서 호출.
- 배치 직후 위/측면 스크린샷 저장 (박스가 실제로 트렁크 안, 깊은 벽에 붙은 자리에 있는지
  육안 확인용 - 수치만 믿지 않는다).
- 원본 12번 스크립트 끝의 "무한 대기 + 라이브 스크린샷" 루프는 배치 테스트용이 아니므로 제거하고,
  포인트클라우드/메타 저장 직후 simulation_app.close()로 자동 종료하도록 바꿈.

그 외 로직(그리퍼 은닉, 카메라 정렬, 스윕 시퀀스)은 12번과 동일.
"""
...
# ---------------- 테스트용 장애물 박스 (깊은 벽에 붙여서 배치) ----------------
# 실제 적재 대상 박스 규격(4.add_boxes_and_zones.py의 "Small") 재사용.
# (x=깊이방향, y=폭방향, z=높이) - 깊은 벽(x_max) 근처라 x폭을 짧게, y폭을 길게 눕혀서
# 스캔 카메라(입구 쪽에서 깊은 곳을 바라봄) 시야에 옆면이 넓게 잡히게 한다.
TEST_BOX_SIZE = (0.20, 0.30, 0.15)  # (x, y, z) meters
TEST_BOX_GAP_FROM_WALL = 0.02  # 깊은 벽면과 박스 사이 여유 (완전히 겹치지 않도록)
TEST_BOX_COLOR = (0.85, 0.55, 0.10)
...
def add_static_test_box(stage, prim_path, size, color):
    """트렁크 깊은 벽(x_max)에 거의 붙여서, y 중앙, 바닥에 놓는 정적 테스트 박스.
    cart/vehicle과 동일하게 물리는 CollisionAPI만 적용(RigidBodyAPI 없음, 낙하/정착 불필요 -
    깊은 벽 바로 앞 바닥 높이에 좌표를 직접 지정해서 놓는다)."""
    x_size, y_size, z_size = size
    center = (
        TRUNK_X_MAX - TEST_BOX_GAP_FROM_WALL - x_size / 2.0,
        0.0,
        TRUNK_FLOOR_Z + z_size / 2.0,
    )
    ...
    prim = cube.GetPrim()
    UsdPhysics.CollisionAPI.Apply(prim)
    print(f"[TEST BOX] {prim_path} size={size} center={center} "
          f"(깊은 벽 x_max={TRUNK_X_MAX} 기준 gap={TEST_BOX_GAP_FROM_WALL})", flush=True)
    return prim
...
add_static_test_box(stage, "/World/TestBox_TrunkBack", TEST_BOX_SIZE, TEST_BOX_COLOR)
...
# 테스트 박스가 실제로 트렁크 안, 깊은 벽에 붙은 자리에 놓였는지 육안 확인 (수치만 믿지 않는다).
# TRUNK_X_MIN/MAX/FLOOR_Z는 이미 월드 좌표(anchor_pos 등에도 그대로 쓰임)이므로 CAR_POS를 더하지 않는다.
set_camera_view(
    eye=[TRUNK_X_MAX - 1.6, -1.4, TRUNK_FLOOR_Z + 1.2],
    target=[TRUNK_X_MAX, 0.0, TRUNK_FLOOR_Z + 0.2],
)
for _ in range(20):
    world.step(render=True)
vp_util.capture_viewport_to_file(vp_util.get_active_viewport(), str(POSE_CHECK_DIR / "test_box_placement_check.png"))
for _ in range(5):
    world.step(render=True)
print("[테스트박스 배치 스크린샷] test_box_placement_check.png", flush=True)
...
print(f"\n[배치 테스트 완료] {RUN_DIR.name} - 포인트클라우드/메타 저장됨. "
      "이 스크립트는 대화형 대기 루프 없이 여기서 바로 종료한다 (13.export_trunk_map.py로 이어서 처리).", flush=True)
