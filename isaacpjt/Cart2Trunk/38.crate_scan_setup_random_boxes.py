"""
38.crate_scan_setup_random_boxes.py
35.crate_scan_setup.py의 복사본 - 35.py는 그대로 두고(잘 동작하는 버전이라 보존),
여기서만 테이블 위 3개 박스(Small/Medium/Large)의 치수를 매 실행마다 무작위로
흔든다(TABLE_BOX_BASE_SIZES 기준 ±TABLE_BOX_SIZE_JITTER). 그 외 로직은 35.py와
동일 - 아래 문서는 35.py 원본 설명 그대로.
...
#   HEADLESS=1 isaac_python 38.crate_scan_setup_random_boxes.py
...
TABLE_BOXES = [
    ("Small", _jittered_size(TABLE_BOX_BASE_SIZES["Small"]), (0.85, 0.25, 0.20), (-0.14, -0.08)),
    ("Medium", _jittered_size(TABLE_BOX_BASE_SIZES["Medium"]), (0.25, 0.65, 0.30), (0.14, -0.08)),
    ("Large", _jittered_size(TABLE_BOX_BASE_SIZES["Large"]), (0.20, 0.35, 0.85), (0.0, 0.10)),
]
print(f"[박스 크기] Small={TABLE_BOXES[0][1]} Medium={TABLE_BOXES[1][1]} Large={TABLE_BOXES[2][1]}", flush=True)
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"38.crate_scan_setup_random_boxes.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_38scan_random"
...
print("\n[완료] 38.crate_scan_setup_random_boxes.py 끝.\n", flush=True)
