...
#
# 35.py에서 더미 박스를 모서리 쪽으로 재배치 + 크기를 0.12->0.15로 키운 뒤 다시
# 스캔+파이프라인을 돌려서 얻은 최신 값(장애물 검출 오차가 0.055m->0.149m로 크게
# 개선됨, 실측 확인) - 알고리즘이 고른 극점 좌표가 이전 레이아웃과 거의 같은 자리로
# 나와서(우연이 아니라 같은 그리디 전략이 비슷한 위치를 고른 것) 값 자체는 이전과
# 큰 차이가 없다. 안쪽으로 당기는 안전 마진은 그대로 유지 - 이번엔 벽에 거의 닿는
# 좌표가 "장애물"이 아니라 크레이트 자체의 하드코딩된 벽이라 실제로는 안전하지만,
# place_hover/release의 팔 오차(0.4-0.5m) 자체는 여전히 남아있어 마진을 벗기지 않는다.
_PLACE_X_RAW, _PLACE_Y_RAW = 1.1848414641051181, 0.10533041038490859
PLACE_WORLD_XY = (_PLACE_X_RAW - 0.15, _PLACE_Y_RAW - 0.08)
PLACE_DIMENSIONS = (0.2803759502533425, 0.1925053843922261, 0.17085018195306662)  # world (x폭, y깊이, 높이)
CRATE_FLOOR_WORLD_Z = 0.14969817408618785  # 크레이트 바닥 (35.py의 CRATE_FLOOR_TOP_Z와 동일, 낮춘 값)
