...
# 적재 알고리즘(algorism/18_rotation.py, ⑱)이 정자세로 안 들어가 90도 돌린 자세로
# 배치를 계산한 경우(placement_result.json의 "rotated": true) - 그리퍼는 흡착으로
# 박스를 뻣뻣하게 붙잡고 있으므로(FixedJoint), 손목을 세계 Z축 기준으로 90도
# 더 돌리면 박스도 그대로 같이 돌아간다. yaw만 다르고 "아래를 본다"는 자세는
# DOWN_QUAT와 동일 - euler_angles_to_quat가 extrinsic 순서로 적용하므로 세 번째
# 성분(yaw)만 다르게 주면 "아래를 보는 채로 세계 Z축 기준 90도 돌아간" 자세가 된다.
...
for idx, placement in enumerate(placements):
    ...
    # 적재 알고리즘이 이 박스는 90도 회전한 자세로 배치해야 자리가 난다고 판단했는지
    # (algorism/18_rotation.py, ⑱) - placement["dimensions"]는 이미 회전을 반영한
    # 값이라 위치/크기 계산은 그대로 맞지만, 실제로 박스를 돌려서 내려놓는 동작
    # 자체는 여기서 명시적으로 만들어줘야 한다.
    ...
    # 회전이 필요하면(rotated=True) 여기서 손목만 90도 돌린다 - 위치는 그대로 두고
    # 방향만 바꿔서(같은 "한 웨이포인트=한 종류 동작만" 원칙, PLACE 접근을 순수 수평/
    # 수직으로만 나눈 것과 같은 이유) 회전과 이동이 한 번에 겹치지 않게 한다.
    if rotated:
        move_link6(world, controller, robot, post_grasp_lift_pos, steps=200, hold_gripper_closed=True,
