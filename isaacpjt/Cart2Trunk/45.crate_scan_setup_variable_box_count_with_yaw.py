"""
45.crate_scan_setup_variable_box_count_with_yaw.py
44.crate_scan_setup_variable_box_count.py의 복사본 - 35~44.py는 그대로 두고(전부 잘
동작하는 버전이라 보존), 여기서는 테이블 박스가 항상 축 정렬(yaw=0)로만 스폰되던 것을
깨고 각 박스가 랜덤 yaw(z축, xy평면 회전 - 이 그리퍼는 항상 수직 하향 흡착이라 pitch/roll은
의미 없음)로 스폰되게 한다. 목적: ① Vision(box_top_extractor.py)이 회전된 박스도 정확한
위치/크기/각도로 검출하는지, ② 적재 알고리즘(19.py, algorism/14.py 복사본)이 그 정보를
받아 최종 목표 각도를 낼 수 있는지, ③ 그리퍼(46.py, 36.py 복사본)가 (박스의 원래 각도 +
알고리즘이 원하는 회전)을 반영해 정확한 최종 각도로 내려놓을 수 있는지 확인하기 위함.

[44.py 대비 달라진 점] (44.py 자체는 안 건드림)
- Vision(box_top_extractor.py)은 이미 cv2.minAreaRect()로 회전된 사각형을 피팅하므로
  corners_m 자체는 손댈 필요가 없다 - 문제는 다운스트림에서 axis-aligned bounding box
  (min/max)로 크기를 재계산하던 부분(44.py의 _matches_known_table_box 등)이 회전된
  박스에서는 실제보다 부풀려진 크기를 낸다는 것. 그래서 이 스크립트에서는:
  - add_dynamic_box()에 yaw_deg 파라미터 추가, xformOp 순서를 Translate->RotateZ->Scale로
    바꿔서 박스 자신의 중심을 축으로 회전하게 한다.
  - TABLE_BOX_YAW_RANGE_DEG 범위에서 박스별 랜덤 yaw를 샘플링(위치보다 먼저 샘플링 -
    겹침 판정에 회전 후 AABB가 필요하므로).
  - 배치 겹침 판정(_sample_box_offset)을 raw (w,d) 대신 회전 후 AABB half-extent로 교체.
  - 크기/각도 매칭을 AABB가 아니라 "정렬된 코너의 변 길이"(_oriented_footprint)로 계산.
- 그 외 로직(더미 장애물 2개, 스캔/필터/저장)은 44.py와 완전히 동일 - 아래 문서는 44.py
  원본 설명 그대로.
...
#   HEADLESS=1 isaac_python 45.crate_scan_setup_variable_box_count_with_yaw.py
...
# [45.py] 테이블 박스가 yaw로 회전되면 실제 점유 영역(axis-aligned bounding box)이
# raw (w,d)보다 커진다 - 겹침 판정에 이 회전 후 AABB를 써야 물리적으로 실제 안 겹치는
# 것을 보장할 수 있다(더미 장애물은 회전이 없으므로 _box_footprint_rect를 그대로 씀).
def _rotated_half_extent(w, d, yaw_deg):
    theta = np.radians(yaw_deg)
    hw, hd = w / 2.0, d / 2.0
    ext_x = hw * abs(np.cos(theta)) + hd * abs(np.sin(theta))
    ext_y = hw * abs(np.sin(theta)) + hd * abs(np.cos(theta))
    return ext_x, ext_y


def _rotated_box_footprint_rect(center_xy, size_wd, yaw_deg):
    cx, cy = center_xy
    ext_x, ext_y = _rotated_half_extent(size_wd[0], size_wd[1], yaw_deg)
    return (cx - ext_x, cx + ext_x, cy - ext_y, cy + ext_y)
...
# ================= 헬퍼 (32.py에서 재사용) =================
def add_dynamic_box(stage, prim_path, center, size, color, mass_kg, yaw_deg=0.0):
    """44.py 대비 yaw_deg 파라미터 추가 - USD는 xformOpOrder에 먼저 추가된 op가 로컬
    좌표에 나중에(부모에 가깝게) 적용되므로, Translate를 먼저 추가하고 그 다음
    RotateZ, 마지막으로 Scale을 추가하면 실제 적용 순서는 "Scale(원점 중심 크기 조정)
    -> RotateZ(원점=박스 자신의 중심 기준 yaw 회전) -> Translate(월드 위치로 이동)"가
    되어, 회전축이 정확히 박스 자신의 중심을 지난다."""
    ...
    print(f"[BOX] {prim_path} center={center} size={size} yaw_deg={yaw_deg:.2f}", flush=True)
...
# [45.py] 박스가 축 정렬(yaw=0)로만 놓이지 않고 이 범위에서 랜덤 yaw로 회전된 채
# 스폰된다 - 이 그리퍼는 항상 수직 하향 흡착이라 pitch/roll은 의미가 없고 yaw(z축)만
# 다룬다. 너무 크게(예: ±90 근방까지) 잡으면 회전 후 AABB가 크게 부풀어 배치 실패가
# 잦아지므로, 우선 완만한 범위로 시작 - 필요하면 이 값만 넓히면 된다.
TABLE_BOX_YAW_RANGE_DEG = (-30.0, 30.0)


def _sample_box_offset(name, size_wd, yaw_deg, placed_rects):
    x_min, x_max, y_min, y_max = SAFE_PLACEMENT_RECT
    ext_x, ext_y = _rotated_half_extent(size_wd[0], size_wd[1], yaw_deg)
    lo_x, hi_x = x_min + ext_x, x_max - ext_x
    lo_y, hi_y = y_min + ext_y, y_max - ext_y
    if lo_x >= hi_x or lo_y >= hi_y:
        print(f"[경고] {name}: SAFE_PLACEMENT_RECT가 이 박스의 회전({yaw_deg:.1f}°) 후 "
              f"크기({2*ext_x:.3f}x{2*ext_y:.3f})보다 작아 샘플링 불가 - 기존 고정 위치로 대체", flush=True)
        return TABLE_BOX_NOMINAL_OFFSETS[name]

    for _attempt in range(PLACEMENT_MAX_ATTEMPTS):
        cx = random.uniform(lo_x, hi_x)
        cy = random.uniform(lo_y, hi_y)
        rect = _rotated_box_footprint_rect((cx, cy), size_wd, yaw_deg)
...
for _name in _placement_order:
    ...
    # yaw를 위치보다 먼저 뽑는다 - 위치 샘플링(겹침 판정)이 회전 후 AABB를 알아야 하므로.
    _yaw = random.uniform(*TABLE_BOX_YAW_RANGE_DEG)
    _sampled_yaws[_name] = _yaw
    _offset = _sample_box_offset(_name, _size, _yaw, _placed_rects)
    _sampled_offsets[_name] = _offset
    _world_center = (CART_POS[0] + _offset[0], CART_POS[1] + _offset[1])
    _placed_rects.append(_rotated_box_footprint_rect(_world_center, (_size[0], _size[1]), _yaw))
    print(f"[박스 위치] {_name} offset={tuple(round(v, 4) for v in _offset)} yaw={_yaw:.2f}°", flush=True)
...
for name, size, color, (dx, dy) in TABLE_BOXES:
    add_dynamic_box(
        ...
        color,
        BOX_MASS_KG[name],
        yaw_deg=_sampled_yaws[name],
    )
    # [진단] 스폰 직후 실제 USD 트랜스폼(로컬 -X~+X 변, 곧 "width" 축)이 정말
    # yaw_deg만큼 world에서 회전됐는지 직접 읽어서 확인한다 - Vision 검출 각도가
    # 샘플링 각도와 어긋나는 게 스폰 자체의 문제인지, Vision 재구성 단계의 문제인지
    # 구분하기 위함(사용자가 PICK 시 회전이 이상하다고 보고해서 확인 중).
    _diag_prim = stage.GetPrimAtPath(f"/World/Box_{name}")
    _diag_mat = UsdGeom.XformCache().GetLocalToWorldTransform(_diag_prim)
    _diag_c0 = np.array(_diag_mat.Transform(Gf.Vec3d(-0.5, -0.5, 0.0)))
    _diag_c1 = np.array(_diag_mat.Transform(Gf.Vec3d(0.5, -0.5, 0.0)))
    _diag_actual_yaw = np.degrees(np.arctan2(_diag_c1[1] - _diag_c0[1], _diag_c1[0] - _diag_c0[0])) % 180.0
    print(f"[진단] {name} 로컬 -X~+X 변(=width 축)의 실제 world yaw={_diag_actual_yaw:.2f}° "
          f"(의도={_sampled_yaws[name] % 180.0:.2f}°)", flush=True)
    for _ in range(100):
        world.step(render=True)
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"45.crate_scan_setup_variable_box_count_with_yaw.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
if _rgba is not None and _rgba.size > 0:
    ...
    print(f"[SCREENSHOT] {_rgb_path.name} (카메라 RGB + ROI 빨간 테두리, {_w}x{_h}, "
          f"ROI px=({_x1},{_y1})-({_x2},{_y2}) - 랜덤 배치/회전된 박스 {NUM_TABLE_BOXES}개가 전부 테두리 안에 있는지 확인용)",
          flush=True)
else:
...
# [45.py] 박스가 회전(yaw)되면 axis-aligned bounding box(AABB, max-min)가 실제
# 크기보다 부풀어 오른다(예: 정사각형을 45도 돌리면 AABB가 대각선 길이가 됨) - 44.py
# 까지는 항상 yaw=0이라 AABB=실제 크기였지만, 여기서는 그 가정이 깨진다. 대신
# box_top_extractor.py가 corners_m[:4](윗면)를 order_rectangle_corners()로 일관된
# 회전 순서로 정렬해서 저장한다는 점을 이용해, 인접한 두 코너 사이의 실제 변 길이로
# 폭/깊이를 계산하고, 그 변의 각도(yaw)도 같이 뽑는다 - 회전과 무관하게 항상 정확하다.
def _oriented_footprint(corners_m):
    ...
    # 사각형은 180도 대칭(뒤집어도 같은 발자국)이므로 yaw는 [0,180)로 정규화한다.
    yaw_deg = float(np.degrees(np.arctan2(edge01[1], edge01[0])) % 180.0)
    return width, depth, yaw_deg


def _oriented_footprint_world(box):
    """_oriented_footprint를 world 좌표계 기준으로 계산한다. box_top_extractor.py의
    corners_m은 base_link 좌표계인데, 로봇이 FACE_ROT_Z(90도)만큼 돌아서 서 있으므로
    base_link 자체가 world와 90도 어긋나 있다 - _world_center()가 위치에 이미 적용하는
    R_base 회전을 각도 계산에도 똑같이 적용해야 world 기준 샘플링 yaw와 공정하게
    비교할 수 있다(이전 버전은 이 회전을 빼먹고 base_link 각도를 world 각도인 것처럼
    비교해서, 실제로는 정확한 검출도 로봇의 90도 회전만큼 어긋난 것처럼 잘못 보였다)."""
    top_world = np.asarray(box["corners_m"][:4], dtype=float) @ R_base.T + base_pos
    return _oriented_footprint(top_world)


def _matches_known_table_box(box):
    width, depth, _yaw_deg = _oriented_footprint(box["corners_m"])
    footprint = tuple(sorted((width, depth)))
    size_ok = any(
...
# [45.py 검증] 이번 실행에서 실제로 샘플링한(스폰에 쓴) 각 박스의 world 위치/크기/yaw와,
# 방금 스캔이 검출해낸 결과를 직접 대조한다 - 회전된 박스도 위치/크기/각도가 전부
# 정확히 검출되는지 로그 한 번으로 확인하기 위함(45.py의 핵심 검증 포인트).
print("\n[검증: 테이블 박스 샘플링 vs 검출(위치/크기/yaw)] ----------------------------------------", flush=True)
for _name in TABLE_BOX_NAMES:
    _gdx, _gdy = _sampled_offsets[_name]
    _gx, _gy = CART_POS[0] + _gdx, CART_POS[1] + _gdy
    _gw, _gd, _gh = TABLE_BOX_SIZES[_name]
    _gyaw = _sampled_yaws[_name] % 180.0
    _best_box, _best_dist = None, None
    for _b in table_real_boxes:
        _bx, _by, _ = _world_center(_b)
        _dist = ((_bx - _gx) ** 2 + (_by - _gy) ** 2) ** 0.5
        if _best_dist is None or _dist < _best_dist:
            _best_box, _best_dist = _b, _dist
    if _best_box is None:
        print(f"[검증] {_name}: 샘플링 center=({_gx:.3f},{_gy:.3f}) size=({_gw:.3f}x{_gd:.3f}) yaw={_gyaw:.1f}° "
              f"-> 대응하는 검출 결과 없음(미검출이거나 크기가 허용치 넘게 다른 후보뿐)", flush=True)
        continue
    _dw, _dd, _dyaw = _oriented_footprint_world(_best_box)
    _dx_world, _dy_world, _ = _world_center(_best_box)
    # yaw는 180도 대칭이라 (target-source) 차이를 [-90,90] 범위로 정규화해서 비교한다.
    _yaw_err = ((_dyaw - _gyaw + 90.0) % 180.0) - 90.0
    print(f"[검증] {_name}: 샘플링 center=({_gx:.3f},{_gy:.3f}) size=({_gw:.3f}x{_gd:.3f}) yaw={_gyaw:.1f}° "
          f"vs 검출 center=({_dx_world:.3f},{_dy_world:.3f}) size=({_dw:.3f}x{_dd:.3f}) yaw={_dyaw:.1f}° "
          f"위치오차={_best_dist:.3f}m yaw오차={_yaw_err:+.1f}°", flush=True)
print("--------------------------------------------------------------------------------------------\n", flush=True)
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_45scan_variable_box_count_with_yaw"
...
print("\n[완료] 45.crate_scan_setup_variable_box_count_with_yaw.py 끝.\n", flush=True)
