"""
43.crate_scan_setup_stacked_verify.py
42.crate_scan_setup_table_box_verify.py의 복사본 - 35/38/39/41/42.py는 그대로
두고(전부 잘 동작하는 버전이라 보존), 여기서는 **적층(stacking) 시나리오**를
추가한다: 40.vision_roi_rectfill_upgrade.py가 시도했던 "Small을 Large 위에
스택"을 다시 가져오되, 이번엔 42.py의 위치/크기 랜덤화 + 실측 비교 인프라 위에서
한다 - 지난 40.py 디버깅(perception/STACKED_BOX_DETECTION_DEBUG_GUIDE.md)에서
"Small이 일부 프레임에서 후보로도 안 잡힘" 미해결 이슈를 다양한 랜덤 배치로
재현/추적하기 위함.

[42.py 대비 달라진 점]
- Large를 먼저 샘플링/드롭+정착시킨 뒤, Small은 독립된 테이블 자리 대신 **Large의
  footprint 안에서** (dx,dy)를 샘플링해서 그 위로 떨어뜨린다 - 물리적으로 자연스럽게
  쌓이게(40.py와 같은 방식, 순서만 명시적으로 강제). Medium은 기존처럼 테이블 위
  독립된 자리에 배치.
- "샘플링 vs 검출" 비교 로그가 XY 거리만 보면 스택된 Small/Large를 혼동할 수 있어서
  (거의 같은 XY, 다른 Z) - Z 높이도 같이 봐서 구분하도록 매칭 로직을 고쳤다.
- 터미널 B에서 `CART2TRUNK_DEBUG_SUPPORT=1`을 export하면 어느 단계에서 후보가
  기각되는지 상세 로그가 뜬다(가이드 문서에 있던 진단 방법 그대로).

[41.py 대비 달라진 점] (42.py에서 이어짐)
- DUMMY_BOX_SIZE(고정 0.15x0.15x0.15, 둘 다 동일)를 박스별 독립 랜덤 크기로 바꿨다
  (테이블 박스와 같은 ±DUMMY_BOX_SIZE_JITTER 지터 방식).
- 더미 위치도 "양쪽 벽에 각각 붙임" 고정 공식 대신, 크레이트 내부 영역
  (CRATE_INNER_SIZE, 벽 여유 DUMMY_MARGIN 유지) 안에서 거부 샘플링으로 뽑는다 -
  둘이 서로 안 겹치게 최소 간격(DUMMY_MIN_GAP_M)을 둔다. 카메라 ROI 계산은
  필요 없다 - 더미는 RANSAC+이미지 ROI가 아니라 포인트클라우드+"이미 아는 크레이트
  영역" 기반으로 검출되므로, 크레이트 벽 안쪽에만 있으면 검출 자체는 보장된다.
- 그 외 로직(테이블 박스 위치/크기 랜덤화, 스캔/필터/저장)은 39.py와 동일 -
  아래 문서는 35.py 원본 설명 그대로.
...
#   HEADLESS=1 isaac_python 43.crate_scan_setup_stacked_verify.py
...
# 3종류만 반복 테스트하지 않고 다양한 크기 조합에서도 적재 알고리즘이 잘 동작하는지
# 보기 위함). w/d/h를 각각 독립적으로 흔들어서 정육면체에 가까운 형태도 나올 수
# 있게 한다. 재현하고 싶으면(같은 크기 조합 다시 보기) 이 줄 위에
# random.seed(원하는 정수)를 추가하면 된다.

random.seed(7)  # 재현용 - 다른 seed를 쓰면 다른 크기 조합이 나온다

TABLE_BOX_BASE_SIZES = {
    "Small": (0.13, 0.10, 0.08),
    "Medium": (0.18, 0.13, 0.12),
    "Large": (0.23, 0.16, 0.14),
}
...
TABLE_BOX_COLORS = {
    "Small": (0.85, 0.25, 0.20),
    "Medium": (0.25, 0.65, 0.30),
    "Large": (0.20, 0.35, 0.85),
}
# 38.py의 원래 고정 오프셋 - 위치 샘플링이 실패했을 때(ROI/겹침/테이블 범위를 만족하는
# 자리를 못 찾았을 때) 그 박스만 이 값으로 되돌아간다(전체를 실패시키지 않는다).
TABLE_BOX_NOMINAL_OFFSETS = {
    "Small": (-0.14, -0.08),
    "Medium": (0.14, -0.08),
    "Large": (0.0, 0.10),
}
TABLE_BOX_SIZES = {
    "Small": _jittered_size(TABLE_BOX_BASE_SIZES["Small"]),
    "Medium": _jittered_size(TABLE_BOX_BASE_SIZES["Medium"]),
    "Large": _jittered_size(TABLE_BOX_BASE_SIZES["Large"]),
}
print(f"[박스 크기] Small={TABLE_BOX_SIZES['Small']} Medium={TABLE_BOX_SIZES['Medium']} "
      f"Large={TABLE_BOX_SIZES['Large']}", flush=True)
...
# [43.py 적층] Small은 테이블 위 독립 자리가 아니라 Large 위에 스택된다(40.py와
# 같은 조합) - Large의 footprint 안에서 (dx,dy)를 뽑아서 벗어나지 않게 한다.
STACK_BASE_NAME = "Large"
STACK_TOP_NAME = "Small"


def _sample_stacked_offset(base_offset, base_size, stacked_size):
    base_dx, base_dy = base_offset
    base_w, base_d = base_size[0], base_size[1]
    stacked_w, stacked_d = stacked_size[0], stacked_size[1]

    base_cx, base_cy = CART_POS[0] + base_dx, CART_POS[1] + base_dy
    lo_x, hi_x = base_cx - base_w / 2.0 + stacked_w / 2.0, base_cx + base_w / 2.0 - stacked_w / 2.0
    lo_y, hi_y = base_cy - base_d / 2.0 + stacked_d / 2.0, base_cy + base_d / 2.0 - stacked_d / 2.0

    # SCAN_EYE가 -y쪽에서 테이블을 내려다보므로(카메라가 항상 -y 쪽에 있음), Small을
    # Large 발자국의 먼 쪽(+y, 카메라 반대편)에 놓으면 Large 자신의 높이에 가려 카메라
    # 시야에서 안 보일 수 있다 - y 샘플링 범위를 카메라와 가까운(-y) 절반으로 제한한다.
    hi_y = min(hi_y, base_cy)

    if lo_x >= hi_x or lo_y >= hi_y:
        # 스택될 박스가 base보다 커서(랜덤 크기 조합이 안 좋게 나온 경우), 혹은 카메라
        # 쪽 절반이 너무 좁아서 footprint 안에서 못 뽑는다 - base 중앙에 그대로 맞춰서
        # 최대한 안 벗어나게라도 놓는다.
        print(f"[경고] {STACK_TOP_NAME}을(를) {STACK_BASE_NAME}의 카메라 쪽 절반 안에서 "
              f"샘플링 불가({stacked_w:.3f}x{stacked_d:.3f} vs {base_w:.3f}x{base_d:.3f}) "
              f"- {STACK_BASE_NAME} 중앙에 정렬", flush=True)
...
# Small(스택 상단)은 독립 샘플링 대상이 아니므로 이 루프에서 빠진다.
_placement_order = [STACK_BASE_NAME, "Medium"]
_sampled_offsets = {}
_placed_rects = []
...
_sampled_offsets[STACK_TOP_NAME] = _sample_stacked_offset(
    _sampled_offsets[STACK_BASE_NAME], TABLE_BOX_SIZES[STACK_BASE_NAME], TABLE_BOX_SIZES[STACK_TOP_NAME]
)
print(f"[박스 위치] {STACK_TOP_NAME} offset="
      f"{tuple(round(v, 4) for v in _sampled_offsets[STACK_TOP_NAME])} ({STACK_BASE_NAME} 위에 스택)",
      flush=True)
...
# 스택이 실제로 성립하려면 Large가 먼저 떨어져 완전히 정착한 뒤에 Small이 그 위로
# 떨어져야 한다. Small까지 다른 박스와 똑같이 TABLE_DROP_Z(테이블 위 0.5m 고정)에서
# 낙하시키면 Large 윗면까지 상당한 자유낙하 속도가 붙은 채로 부딪혀 모서리에서
# 튕기거나 미끄러져 테이블 바닥에 안착해버린다(40.py가 Small 전용으로 Large 윗면
# 바로 위 짧은 거리에서 낙하시키는 특수 케이스를 둔 이유 - 43.py는 그 특수 케이스를
# 빠뜨리고 있었음).
#
# world.reset()이 아직(로봇 초기화 시점에) 호출되기 전이라 이 시점의 world.step()은
# 물리 타임라인을 진행시키지 않는다(diagnostic으로 확인: 이 시점 current_time=0.000s,
# 즉 Large는 이 시점에 아직 실제로 떨어지지 않은 spawn 높이 그대로다). 그래서 아래
# "Large 실측 윗면"은 사실 Large의 spawn 높이(TABLE_DROP_Z)일 뿐 진짜 정착 높이가
# 아니다 - 그런데도 안전한 이유: Small의 spawn_z = (Large spawn 높이) + Large_height +
# Small_height/2 + margin 이라, world.reset() 이후 모든 박스가 동시에 자유낙하를
# 시작해도 Small은 항상 Large보다 최소 "Large_height + margin"만큼 높은 곳에서
# 출발한다(박스 크기 랜덤 jitter와 무관하게 항상 성립) - 동일 중력 가속도로 떨어지므로
# 이 높이차가 낙하 내내 유지되다가, Large가 먼저 테이블에 닿아 멈추면 Small은 그 差만큼
# 남은 거리를 마저 떨어져 Large 위에 안착한다.
_STACK_TOP_SPAWN_MARGIN_M = 0.05
_table_boxes_by_name = {name: (size, color, offset) for name, size, color, offset in TABLE_BOXES}
_stack_base_top_z_at_spawn = None
for name in (STACK_BASE_NAME, "Medium", STACK_TOP_NAME):
    size, color, (dx, dy) = _table_boxes_by_name[name]
    if name == STACK_TOP_NAME:
        spawn_z = _stack_base_top_z_at_spawn + size[2] / 2.0 + _STACK_TOP_SPAWN_MARGIN_M
    ...
    if name == STACK_BASE_NAME:
        _stack_base_top_z_at_spawn = spawn_z + size[2] / 2.0
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"43.crate_scan_setup_stacked_verify.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
print("\n[대기] 지금 별도 터미널에서 다음을 실행하세요 (venv/ROS2 환경 설정 포함):\n"
      ...
      "  (저비용 검증: 여러 프레임 point cloud를 합쳐서 한 번만 검출하는 실험을 하려면\n"
      "   run_scan_once.py 대신 run_scan_once_accumulate.py를 그대로 같은 --marker로 실행)\n",
      flush=True)
...
# [43.py 적층 대응] XY만 보면 Small이 Large 바로 위에 스택돼 있을 때 "같은 물리
# 박스가 중복 검출된 것"으로 오판해 실제로 서로 다른 두 박스 중 하나를 지워버린다
# (가이드 문서에 기록된 바로 그 실수) - Z도 같이 가까울 때만(AND 조건) 중복으로
# 본다. 서로 쌓여있어 XY는 같지만 Z가 뚜렷이 다른 경우는 절대 같은 박스가 아니다.
...
# [43.py 검증] 42.py의 "샘플링 vs 검출" 비교를 그대로 쓰되, 이번엔 Small이 Large
# 위에 스택돼서 둘의 XY가 거의 같다 - XY만 보고 매칭하면 Small의 검출 결과를 Large로
# (혹은 그 반대로) 잘못 매칭할 위험이 있다(가이드 문서의 "dedup이 Z를 무시해서
# 헷갈렸다"는 것과 같은 종류의 함정). 그래서:
#   1. 각 박스의 실측 world Z 중심을 스택 관계까지 반영해서 미리 계산해두고
#      (Small=Large 위, Large/Medium=테이블 위) 매칭을 3D(XY+Z) 거리로 한다.
#   2. 이미 다른 박스에 매칭된 검출 후보는 재사용하지 않는다(한 후보가 Small/Large
#      둘 다에 "제일 가깝다"고 중복 매칭되는 것 방지).
_table_box_world_z_center = {
    "Medium": TABLE_TOP_Z + TABLE_BOX_SIZES["Medium"][2] / 2.0,
    STACK_BASE_NAME: TABLE_TOP_Z + TABLE_BOX_SIZES[STACK_BASE_NAME][2] / 2.0,
    STACK_TOP_NAME: TABLE_TOP_Z + TABLE_BOX_SIZES[STACK_BASE_NAME][2] + TABLE_BOX_SIZES[STACK_TOP_NAME][2] / 2.0,
}

# [버그 수정] 이전 버전은 위치(3D 거리)만 보고 "제일 가까운 후보"를 무조건 매칭했다
# - 크기 필터(TABLE_BOX_SIZE_TOLERANCE_M)를 통과하지 못해 진짜 후보가 빠지면, 남은
# (크기가 전혀 다른) 후보를 억지로 매칭해서 완전히 엉뚱한 비교 결과를 냈다(실측:
# Small 자리에 Large 크기 검출값이 매칭됨). 이제 크기가 크게 다른 후보는 애초에
# 매칭 대상에서 제외한다. 값은 실제 오검출 사례로 직접 검증해서 정함 - 0.05는 너무
# 넉넉해서 Medium 검출값(box0)이 Small 오차범위 안에 들어와 여전히 잘못 매칭됐고,
# 0.03으로 좁히니 Small/Large/Medium 세 조합 모두 올바르게 갈렸다(크기 필터의
# 0.025보다는 살짝 넉넉해서 "위치는 조금 어긋났지만 크기는 맞는" 진짜 오차 케이스는
# 여전히 허용한다).
_SIZE_MISMATCH_GATE_M = 0.03

print("\n[검증: 테이블 박스 샘플링 vs 검출] --------------------------------------", flush=True)
_used_detected_ids = set()
# 스택 상단(Small)부터 매칭해야 안전하다 - Large를 먼저 매칭하면 Small의 진짜 검출
# 후보(Large보다 살짝 높은 Z)가 아직 안 쓰인 채 남아있다는 보장이 없다.
for _tname in (STACK_TOP_NAME, STACK_BASE_NAME, "Medium"):
    _tdx, _tdy = _sampled_offsets[_tname]
    _tgx, _tgy = CART_POS[0] + _tdx, CART_POS[1] + _tdy
    _tgz = _table_box_world_z_center[_tname]
    _tgw, _tgd, _tgh = TABLE_BOX_SIZES[_tname]
    _gt_wd_sorted = sorted((_tgw, _tgd))
    _best_b, _best_dist, _best_idx = None, None, None
    for _bi, _b in enumerate(table_real_boxes):
        if _bi in _used_detected_ids:
            continue
        _pts_world = np.asarray(_b["corners_m"]) @ R_base.T + base_pos
        _mn_chk, _mx_chk = _pts_world.min(axis=0), _pts_world.max(axis=0)
        _det_wd_sorted = sorted((_mx_chk[0] - _mn_chk[0], _mx_chk[1] - _mn_chk[1]))
        if (abs(_det_wd_sorted[0] - _gt_wd_sorted[0]) > _SIZE_MISMATCH_GATE_M
                or abs(_det_wd_sorted[1] - _gt_wd_sorted[1]) > _SIZE_MISMATCH_GATE_M):
            continue  # 크기가 너무 달라 애초에 이 박스일 수 없음 - 매칭 후보에서 제외
        _bc = _pts_world.mean(axis=0)
        _dist = ((_bc[0] - _tgx) ** 2 + (_bc[1] - _tgy) ** 2 + (_bc[2] - _tgz) ** 2) ** 0.5
        if _best_dist is None or _dist < _best_dist:
            _best_b, _best_dist, _best_idx = _b, _dist, _bi
    if _best_b is None:
        print(f"[검증] {_tname}: 샘플링 center=({_tgx:.3f},{_tgy:.3f},{_tgz:.3f}) "
              f"size=({_tgw:.3f}x{_tgd:.3f}x{_tgh:.3f}) -> 대응하는 검출 결과 없음"
              f"(미검출이거나 크기가 {_SIZE_MISMATCH_GATE_M}m 넘게 다른 후보뿐)", flush=True)
        continue
    _used_detected_ids.add(_best_idx)
    _pts_world = np.asarray(_best_b["corners_m"]) @ R_base.T + base_pos
    _mn, _mx = _pts_world.min(axis=0), _pts_world.max(axis=0)
    _dims = _mx - _mn
    print(f"[검증] {_tname}: 샘플링 center=({_tgx:.3f},{_tgy:.3f},{_tgz:.3f}) "
          f"size=({_tgw:.3f}x{_tgd:.3f}x{_tgh:.3f}) vs 검출 "
          f"center=({(_mn[0]+_mx[0])/2:.3f},{(_mn[1]+_mx[1])/2:.3f},{(_mn[2]+_mx[2])/2:.3f}) "
          f"size=({_dims[0]:.3f}x{_dims[1]:.3f}x{_dims[2]:.3f}) 3D오차={_best_dist:.3f}m", flush=True)
print("----------------------------------------------------------------------\n", flush=True)
...
# [41.py 검증] 이번 실행에서 실제로 샘플링한(스폰에 쓴) 더미 위치/크기와, 방금 스캔이
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_43scan_stacked_verify"
...
print("\n[완료] 43.crate_scan_setup_stacked_verify.py 끝.\n", flush=True)
