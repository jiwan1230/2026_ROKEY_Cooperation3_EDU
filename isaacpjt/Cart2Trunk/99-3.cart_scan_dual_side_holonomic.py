...
# 사용자 요청(99-3: main 브랜치의 99.cart_scan_dual_side_holonomic.py를 기준으로,
# cart-stack-more-boxes 브랜치에서 지하주차장 USDZ 환경을 병합) - 기준은 무조건
# main의 카트/로봇/박스 환경이고, 주차장은 그 위에 배경으로만 얹는다. 99-2/100-2
# (cart-stack-more-boxes 자체 버전 기준)에서 검증된 것과 완전히 동일한 값/방식 -
# 실측 확인(_view_usdz_env.py로 먼저 단독 로드해서 확인함): src_mpu=0.01, src_up=Y,
# 스케일 적용 후 bbox min=[-14.79,-12.10,-0.11] max=[19.79,11.02,2.89] - 실제
# 주차장 바닥이 기존 환경의 바닥(Z=0)보다 약 0.112m 아래에 있으므로, 배치 시
# 그만큼 위로 올려서 두 바닥을 맞춘다.
...
# 사용자 요청(99-3) - 지하주차장 배경을 카트/로봇보다 먼저 추가한다(순서 자체는
# 중요하지 않음 - 서로 다른 prim path라 간섭 없음).
