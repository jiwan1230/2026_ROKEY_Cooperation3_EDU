...
def run_trunk_scan(timeout_sec: int = TRUNK_SCAN_TIMEOUT_SEC) -> dict:
    ...
    cmd = (
        ...
        f"ros2 run cart2trunk_bridge trunk_scan_client "
    ...
    try:
        ...
        raise RuntimeError(f"트렁크 스캔 서브프로세스 타임아웃: {e}") from e

    stdout_lines = [line for line in proc.stdout.splitlines() if line.strip()]
    if not stdout_lines:
        raise RuntimeError(
            f"트렁크 스캔 서브프로세스가 출력이 없습니다(returncode={proc.returncode}). "
    ...
    try:
        ...
        raise RuntimeError(
            f"트렁크 스캔 결과를 JSON으로 파싱할 수 없습니다: {stdout_lines[-1]!r} ({e})") from e

    if not result.get("success"):
        raise RuntimeError(result.get("message", "트렁크 스캔 실패(원인 불명)"))
