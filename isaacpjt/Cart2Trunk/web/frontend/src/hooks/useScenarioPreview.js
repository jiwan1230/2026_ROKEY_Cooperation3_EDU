...
// 산업현장 시나리오 미리보기 상태 - Scene3DViewer(3D 미리보기), SummaryCard
// (안내 문구), ControlPanel(파라미터 잠금)이 전부 같은 시나리오 상태를 봐야
// 해서 세 컴포넌트의 공통 부모(App.jsx의 PlanningBody)에서 이 훅으로 한 번만
// 만들어 내려준다. PlannerContext(state.result)는 전혀 건드리지 않는 완전히
// 별도 상태다 - 지금 작업 중인 실시간 계획이 시나리오 미리보기 때문에
// 사라지면 안 된다.
