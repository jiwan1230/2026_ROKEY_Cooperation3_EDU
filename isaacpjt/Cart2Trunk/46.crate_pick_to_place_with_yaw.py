"""
46.crate_pick_to_place_with_yaw.py
36.crate_pick_to_place.py의 복사본 - 36.py 자체는 건드리지 않는다.

배경 (45.py/19.py와 짝)
----
36.py는 테이블 박스가 항상 축 정렬(yaw=0)로 스폰된다는 전제 위에서, 적재 알고리즘의
"rotated" 결정(0° 또는 90°)을 그대로 최종 손목 절대각으로 썼다(DOWN_QUAT/
DOWN_QUAT_ROTATED90 이분법). 45.py부터는 박스가 임의 yaw로 스폰되므로, 19.py
(algorism/14.py의 yaw 지원 사본)가 "박스의 원래 on-table yaw + 적재 알고리즘의 목표
절대각"을 미리 합쳐서 계산해준 wrist_yaw_deg(임의 각도)를 그대로 받아 쓴다 -
DOWN_QUAT_ROTATED90을 wrist_quat(yaw_deg) 함수 하나로 일반화한 것 외에는 36.py와
동일한 구조(PICK -> pick_rotate(회전 필요시) -> 리포지셔닝 -> PLACE)를 그대로 따른다.
...
지속 가능하지 않다는 지적(사용자)에 따라, 이 스크립트는 이제:
  ...
  - placement_result.json의 box_id가 어떤 물리 USD 프림인지는 이름표가 아니라
    "스캔된 테이블 위 위치와 가장 가까운 실측 위치의 프림"으로 매칭한다 - 스캔이
    매번 box_id를 카메라 거리순으로 새로 매기므로(Q4, 재스캔해도 같은 id가 보장
    안 됨) 이름 매칭보다 위치 매칭이 안전하다. 후보 프림 목록 자체도 이름을
    하드코딩하지 않고 discover_box_prim_paths()로 씬에서 동적으로 찾는다(아래 참고) -
    그래야 박스 개수/이름이 다른 씬(예: 44.py의 Box1~BoxN)에서도 그대로 동작한다.
...
#   HEADLESS=1 isaac_python 46.crate_pick_to_place_with_yaw.py
...
def wrist_quat(yaw_deg: float):
    """36.py는 DOWN_QUAT(회전 없음)/DOWN_QUAT_ROTATED90(90도) 둘 중 하나만 썼다 -
    45.py부터 테이블 박스가 축 정렬(yaw=0)이 아니라 임의 yaw로 스폰되므로, 19.py
    (algorism/14.py의 yaw 지원 사본)가 계산해서 placement_result.json에 넣어주는
    "wrist_yaw_deg"(임의 각도, 도)를 그대로 받아 손목 자세를 만든다. yaw_deg=0이면
    DOWN_QUAT와 완전히 동일하므로 회전이 필요 없는 박스도 그대로 커버된다.
    euler_angles_to_quat가 extrinsic 순서로 적용하므로 세 번째 성분(yaw)만 바꾸면
    "아래를 보는 채로 세계 Z축 기준 yaw_deg만큼 돌아간" 자세가 된다."""
    return euler_angles_to_quat(np.array([0.0, np.pi, np.radians(yaw_deg)]))
...
# 테이블에 실제로 만들어진 물리 박스들(TABLE_BOXES) - 어떤 placement_result.json
# box_id가 이 중 어느 프림인지는 이름이 아니라 스캔 위치 매칭으로 실행 시점에 정한다
# (match_physical_prim 참고, Q4: 재스캔해도 box_id 지속성이 보장 안 됨).
#
# 후보 프림 목록 자체는 이름을 하드코딩하지 않고, 씬 로드 후 discover_box_prim_paths()로
# 동적으로 찾는다 - 35/38~43.py는 Small/Medium/Large 3개 고정이지만 44.py부터는 박스
# 개수/이름이 스크립트마다 다를 수 있다(Box1~BoxN 등). add_dynamic_box()가 항상
# "/World/Box_<이름>" 규칙으로 프림을 만들므로, 그 규칙만 따라가면 이름 목록을 미리
# 알 필요가 없다.


def discover_box_prim_paths(stage):
    """/World 바로 아래에서 "Box_"로 시작하는 프림을 전부 후보로 찾는다(위 설명 참고)."""
...
def _quat_yaw_deg(gf_quat) -> float:
    """Gf.Quat(d/f)에서 world Z축 기준 yaw(도)를 뽑는다 - 박스는 바닥에 평평하게
    누워있어(pitch/roll 없음) 순수 Z 회전만 가정해도 된다. [진단] 흡착 순간 박스가
    회전이 튀는지 확인하는 용도로만 쓴다."""
    w = gf_quat.GetReal()
    x, y, z = gf_quat.GetImaginary()
...
class DynamicSuctionGripper(SurfaceGripper):
    ...
    def close(self) -> None:
        ...
        if not target_prim.IsValid():
            return
        # [원복 2회차] "스케일 뺀 순수 미터 단위 rel_local"을 시도했다가 오히려
        # 훨씬 심각한 회귀가 났다(각속도가 기존 0.01~0.07 수준에서 [8.8,23.9,11.5]
        # rad/s까지 치솟음, yaw 변화도 +5°->+8.95°로 악화) - 이는 가설이 반대
        # 방향이었다는 명확한 반증이다: PhysX(UsdPhysics 파싱)는 오히려 스케일까지
        # 포함한 원래 방식(GetInverse().Transform(), unit-cube 좌표)을 더 가깝게
        # 기대하고 있었던 것으로 보인다. 원래(스케일 포함) 방식으로 완전히 되돌린다 -
        # 남아있는 국소적 회전 스냅(1~5도)은 이 스케일 문제가 원인이 아니었다는 뜻이므로
        # 다른 가설(예: 조인트 강성/드라이브 자체의 순간 반응)로 접근해야 한다.
        ...
        self._attached = True
        # [진단] 조인트에 "박아넣는" 그 순간 박스의 실제 world yaw를 같이 찍는다 -
        # 이후(attach 루프 종료 후) 다시 잰 yaw와 비교해서 흡착 시점에 회전이
        # 튀는지(staleness) 아니면 그 후에 튀는지(잔류 각속도/조인트 보정) 구분한다.
        print(f"  [흡착] dist={dist:.4f}m <= {self._grasp_radius}m -> {self._joint_path} 생성 "
              f"[진단] 흡착 순간 박스 yaw={_quat_yaw_deg(target_rot):.2f}°", flush=True)
...
def move_link6(world, controller, robot, target_pos, steps=WAYPOINT_STEPS, hold_gripper_closed=False,
               label="", chassis_pin=None, orientation=DOWN_QUAT):
...
for idx, placement in enumerate(placements):
    ...
    # 19.py(algorism/14.py의 yaw 지원 사본)가 이미 "박스의 원래 on-table yaw +
    # 적재 알고리즘의 0°/90° 배치 결정"을 합쳐서 최종 손목 절대각을 계산해뒀다
    # (wrist_yaw_deg) - 45.py부터 테이블 박스가 축 정렬이 아니라 임의 yaw로 스폰되므로
    # 36.py의 "90도냐 아니냐" 이분법 대신 이 임의 각도를 그대로 쓴다. 회전이 거의
    # 없으면(1도 미만) pick_rotate 웨이포인트 자체를 생략해 불필요한 동작을 줄인다.
    wrist_yaw_deg = float(placement.get("wrist_yaw_deg", 0.0))
    box_orientation = wrist_quat(wrist_yaw_deg)
    rotated = abs(wrist_yaw_deg) > 1.0
    if rotated:
        print(f"[회전] box_id={box_id}는 {wrist_yaw_deg:+.1f}° 회전 배치 - PICK 직후 손목을 돌린다", flush=True)
    ...
    # [시도 1] pick_grasp 도달 직후에도 관절 드라이브가 마지막 IK 목표로 계속
    # 미세 수렴 중일 수 있다(move_link6의 마지막 스텝 err가 0에 완전히 도달하지
    # 않음, 로그 실측 0.0009~0.005m) - close()가 그 "아직 살짝 움직이는" 순간의
    # 그리퍼 자세로 조인트 rest frame을 계산하면, 이후 몇 스텝 동안 팔이 마저
    # 수렴하면서 방금 만든 조인트가 박스를 끌고 다녀 회전이 튀는 것으로 추정된다.
    # 같은 목표(pick_grasp_pos, DOWN_QUAT)로 controller.forward()를 계속 호출해서
    # 엔드이펙터 속도가 충분히 가라앉을 때까지 추가로 정지시킨 뒤에만 close()를
    # 시작한다.
    GRASP_SETTLE_STEPS = 60
    for _ in range(GRASP_SETTLE_STEPS):
        actions = controller.forward(
            target_end_effector_position=np.array(pick_grasp_pos, dtype=float),
    ...
    # [진단] 흡착 직전 그리퍼(엔드이펙터)와 박스의 실제 world yaw/속도를 먼저 찍는다 -
    # close() 안의 "흡착 순간 yaw" 로그, 그리고 잠시 뒤(attach 루프 종료 후)의 yaw와
    # 비교해서 어디서 회전이 튀는지(흡착 직전 팔/박스가 이미 움직이고 있었는지,
    # 흡착 읽기 자체가 stale했는지, 흡착 후 조인트가 보정하며 튀는지) 구분한다.
    # robot.end_effector는 SingleManipulator가 이미 올바르게 초기화해둔 것이라
    # (링크를 별도로 감싸는 게 아님) 안전하게 재사용할 수 있다.
    _diag_ee_linvel = robot.end_effector.get_linear_velocity()
    _diag_ee_angvel = robot.end_effector.get_angular_velocity()
    print(f"  [진단] {GRASP_SETTLE_STEPS}스텝 추가 정지 후 엔드이펙터 선속도={np.round(_diag_ee_linvel, 4)} "
          f"각속도={np.round(_diag_ee_angvel, 4)}", flush=True)

    _diag_box_rigid = SingleRigidPrim(prim_path)
    _diag_box_rigid.initialize(physics_sim_view=world.physics_sim_view)
    _diag_mat_before = UsdGeom.Xformable(box_prim).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
    _diag_yaw_before = _quat_yaw_deg(_diag_mat_before.ExtractRotationQuat())
    _diag_angvel_before = _diag_box_rigid.get_angular_velocity()
    print(f"  [진단] 흡착 직전 박스 yaw={_diag_yaw_before:.2f}° 각속도={np.round(_diag_angvel_before, 4)}", flush=True)
    ...
    # [진단] attach 루프(30스텝) 종료 후 박스의 yaw/각속도 - close() 안에서 찍은
    # "흡착 순간 yaw"와 이 값을 비교하면, 흡착 이후에도 박스가 계속 돌았는지 바로 보인다.
    _diag_mat_after = UsdGeom.Xformable(box_prim).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
    _diag_yaw_after = _quat_yaw_deg(_diag_mat_after.ExtractRotationQuat())
    _diag_angvel_after = _diag_box_rigid.get_angular_velocity()
    print(f"  [진단] attach 루프 종료 후 박스 yaw={_diag_yaw_after:.2f}° 각속도={np.round(_diag_angvel_after, 4)} "
          f"(흡착 직전 대비 yaw 변화={((_diag_yaw_after - _diag_yaw_before + 90) % 180) - 90:+.2f}°)", flush=True)
    ...
    # 회전이 필요하면(|wrist_yaw_deg|>1도) 여기서 손목만 wrist_yaw_deg만큼 돌린다 -
    # 위치는 그대로 두고 방향만 바꿔서(같은 "한 웨이포인트=한 종류 동작만" 원칙,
    # PLACE 접근을 순수 수평/수직으로만 나눈 것과 같은 이유) 회전과 이동이 한 번에
    # 겹치지 않게 한다.
    if rotated:
        move_link6(world, controller, robot, post_grasp_lift_pos, steps=200, hold_gripper_closed=True,
                   label=f"pick_rotate[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
    ...
    move_link6(world, controller, robot, carry_pos, steps=200, hold_gripper_closed=True,
               label=f"pick_carry[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
    ...
    move_link6(world, controller, robot, place_approach_pos, steps=300, hold_gripper_closed=True, label=f"place_approach[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
    move_link6(world, controller, robot, place_hover_pos, steps=300, hold_gripper_closed=True, label=f"place_hover[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
    ...
    move_link6(world, controller, robot, place_hover_pos, steps=200, label=f"place_retract_hover[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
    place_retract_safe_pos = (place_world_xy[0], place_world_xy[1], SAFE_TRANSIT_Z)
    move_link6(world, controller, robot, place_retract_safe_pos, steps=200, label=f"place_retract_safe[{idx}]", chassis_pin=chassis_pin, orientation=box_orientation)
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_46pickplace_with_yaw"
