"""
40.vision_roi_rectfill_upgrade.py

35.crate_scan_setup.py를 기반으로 Vision 파라미터를 업그레이드한 버전:

1. ROI: 0.15,0.15,0.85,0.85 (중앙 영역만 사용)
2. 사각형 채움 비율: 0.50 (처음엔 0.60으로 강하게 걸렀다가, 실측으로 Medium/Large의
   정상 윗면이 depth 노이즈발 RANSAC 평면 분할 때문에 fill_ratio 0.52~0.59로 자꾸
   기각되는 걸 확인해서 완화 - perception/box_top_extractor.py 주석 참고)
3. 평면 거리 임계값: 0.008 (기존 0.006 - 위 분할 자체를 줄이기 위해 소폭 완화)
4. 테이블 박스 배치: Medium(앞) + Large(뒤) + Small(Large 위에 스택)

이 파라미터들은 box_top_extractor.py를 실행하는 "별도 터미널/프로세스"의 환경변수로
제어된다 - 이 스크립트(40.py) 자신의 os.environ.setdefault()는 그 별도 프로세스에
전달되지 않는다(서로 다른 OS 프로세스라 환경이 공유되지 않음)는 걸 실측으로 확인했다.
그래서 실제 동작을 결정하는 건 box_top_extractor.py 쪽의 기본값이고, 여기 값은 그
기본값과 반드시 일치시켜야 하는 "의도 문서화"용이다 - 아래 [대기] 안내 문구에도 같은
값을 export하도록 맞춰뒀다(실제로 이 스크립트를 통해 실행할 경우를 위해).
"""
...
# ================= Vision 파라미터 (40번 버전) =================
# 주의: 아래 os.environ.setdefault()는 "이 스크립트 자신의" 프로세스 환경만
# 바꾼다 - box_top_extractor.py는 별도 터미널의 별도 프로세스로 실행되므로 이
# 값을 물려받지 않는다(위 docstring 참고). 실제 값은 box_top_extractor.py 자체의
# 기본값이 결정하며, 여기 값들은 그것과 동기화된 "의도 표시+ [대기] 안내문 참고용"이다.
os.environ.setdefault("CART2TRUNK_IMAGE_ROI", "0.15,0.15,0.85,0.85")
os.environ.setdefault("CART2TRUNK_MIN_RECTANGULAR_FILL_RATIO", "0.50")
os.environ.setdefault("CART2TRUNK_PLANE_DISTANCE_THRESHOLD_M", "0.008")
print("[40] Vision 파라미터 설정 (box_top_extractor.py 기본값과 동기화되어 있어야 함):")
print(f"  - ROI: {os.environ['CART2TRUNK_IMAGE_ROI']}")
print(f"  - Min Rectangular Fill Ratio: {os.environ['CART2TRUNK_MIN_RECTANGULAR_FILL_RATIO']}")
print(f"  - Plane Distance Threshold: {os.environ['CART2TRUNK_PLANE_DISTANCE_THRESHOLD_M']}")
...
# - Medium: 앞쪽(로봇에 더 가깝게)
# - Large: 뒤쪽(+Y)
# - Small: Large 위에 스택 (z_offset 제어)
TABLE_BOXES = [
    ("Medium", (0.18, 0.13, 0.12), (0.25, 0.65, 0.30), (-0.14, -0.04)),  # 앞쪽
    ("Large", (0.23, 0.16, 0.14), (0.20, 0.35, 0.85), (0.0, 0.12)),     # 뒤쪽
    ("Small", (0.13, 0.10, 0.08), (0.85, 0.25, 0.20), (0.0, 0.12)),     # Large 위
]
...
for name, size, color, (dx, dy) in TABLE_BOXES:
    # Small 박스는 Large 위에 스택되도록 z를 조정
    if name == "Small":
        # Large 박스가 먼저 생성되어 테이블에 안착했으므로
        # Large 상단 + Small 높이의 절반으로 충분히 높게 설정
        large_height = 0.14  # Large 박스의 높이
        small_height = 0.08  # Small 박스의 높이
        spawn_z = TABLE_DROP_Z + large_height/2 + small_height/2 + 0.05  # 약간의 여유
        print(f"[Small 스택] Large 위에 배치 (z={spawn_z:.3f})", flush=True)
    else:
...
# Medium이 프레임마다 부분적으로만 관측되는 원인(ROI 크롭이 실제로 Medium을 자르는지,
# 아니면 팔/그리퍼가 시야를 가리는지)을 직접 눈으로 확인하기 위해 카메라 자신의
# RGB 프레임을 ROI 경계선과 함께 저장한다 - box_top_extractor.py가 보는 것과
# 동일한 뷰(같은 camera_prim_path)라 진짜 원인 확인에 직접 쓸 수 있다.
for _ in range(5):
    world.step(render=True)
...
if _rgba is not None and _rgba.size > 0:
    ...
    _h, _w = _rgb.shape[:2]
    _left, _top, _right, _bottom = IMAGE_ROI = tuple(
        float(v) for v in os.environ["CART2TRUNK_IMAGE_ROI"].split(",")
    )
    ...
    _rgb_path = _THIS_DIR / "_debug_table_scan_rgb_roi.png"
    Image.fromarray(_rgb).save(str(_rgb_path))
    print(f"[SCREENSHOT] {_rgb_path.name} (카메라 RGB + ROI 빨간 테두리, {_w}x{_h}, "
          f"ROI px=({_x1},{_y1})-({_x2},{_y2}))", flush=True)
else:
    print("[경고] camera.get_rgba() 가 비어있어 RGB 디버그 스크린샷을 저장하지 못했습니다.", flush=True)
...
print("\n[대기] 지금 별도 터미널에서 다음을 실행하세요 (venv/ROS2 환경 설정 포함):\n"
      ...
      # box_top_extractor.py는 별도 프로세스라 위 os.environ.setdefault()를 물려받지
      # 않는다 - 40번 파라미터를 실제로 적용하려면 여기서도 명시적으로 export해야 한다.
      f"  export CART2TRUNK_IMAGE_ROI={os.environ['CART2TRUNK_IMAGE_ROI']}\n"
      f"  export CART2TRUNK_MIN_RECTANGULAR_FILL_RATIO={os.environ['CART2TRUNK_MIN_RECTANGULAR_FILL_RATIO']}\n"
      f"  export CART2TRUNK_PLANE_DISTANCE_THRESHOLD_M={os.environ['CART2TRUNK_PLANE_DISTANCE_THRESHOLD_M']}\n"
      f"  cd {PERCEPTION_DIR}\n"
...
# RGB로 가림이 아님을 확인한 뒤 fill_ratio(0.60->0.50)/plane_distance(0.006->0.008)를
# 완화했더니 Medium/Large의 진짜 검출은 안정화됐지만, 그 대가로 새로운 오탐이 하나
# 나타났다: 테이블 스캔 카메라 시야 가장자리에 살짝 걸리는 "크레이트"(테이블에서
#0.9m 떨어진 별개 구조물, 원래 이 스캔 포즈에서는 안 보여야 함) 조각이 이제 크기
# 조건(0.16x0.23, Large와 거의 동일)까지 우연히 통과해서 실측 3개 후보 중 하나로
# 끼어들었다(base_link 좌표 (0.666,-0.678) -> world (0.68,0.12) 로 역산해보면 테이블
# 범위(x<=0.4) 밖, 크레이트 범위(x 0.455~1.345) 안 - 진짜 테이블 위 박스가 아님을
# 위치로 확정할 수 있다). 크기만으로는 이런 "크기는 맞는데 엉뚱한 위치" 오탐을 못
# 거른다는 게 이번에 실측으로 드러난 것 - 우리가 이미 아는 TABLE_SIZE/CART_POS
# 범위 안에 있는지(월드 좌표로 환산해서) 확인하는 위치 기반 필터를 크기 필터와
# 함께 적용한다(크레이트 더미 박스를 크기로 걸렀던 것과 같은 "우리가 만든 값은
# 스캔 없이도 검증에 쓸 수 있다"는 패턴의 연장).
TABLE_POSITION_MARGIN_M = 0.05


def _is_on_table_world(box):
    cx, cy, _cz = _table_box_center_xyz(box)
    world_rel = R_base @ np.array([cx, cy, 0.0])
    wx, wy = base_pos[0] + world_rel[0], base_pos[1] + world_rel[1]
    half_w = TABLE_SIZE[0] / 2.0 + TABLE_POSITION_MARGIN_M
    half_d = TABLE_SIZE[1] / 2.0 + TABLE_POSITION_MARGIN_M
    return (
        abs(wx - CART_POS[0]) <= half_w
        and abs(wy - CART_POS[1]) <= half_d
    )


table_boxes_data = json.loads(table_boxes_json_path.read_text())
_table_total = len(table_boxes_data["boxes"])
_size_ok_table_boxes = [
    b for b in table_boxes_data["boxes"]
    if _matches_known_table_box(b) and _is_on_table_world(b)
]
...
#
# 단, 이 프로젝트(40번)는 Small을 Large 위에 스택해서 XY 중심이 사실상 동일하게
# 설정했다(TABLE_BOXES 참고) - XY만 보는 dedup은 "스택된 별개의 두 박스"를
# "같은 박스의 중복 검출"과 구분하지 못해 실제로 하나가 삭제되는 버그가 있었다
# (실측 확인: 적층 상태에서 박스가 1개로만 출력됨). 크레이트 더미 중복은 애초에
# 같은 높이의 같은 물체를 가리키는 두 후보였으므로, Z(높이)도 함께 가까울 때만
# 중복으로 판단하도록 조건을 추가한다 - 원래 유스케이스(같은 높이 중복)는 그대로
# 걸러지고, 스택(다른 높이) 케이스만 살아남는다.
