...
# 더 키운다).
# 큐브를 받침대 가장자리(+0.12)로 옮겼으니, 0.70 그대로면 이제 팔 도달거리가 남는다. 그렇다고
# 너무 줄이면 섀시가 받침대(반너비 0.175)에 다시 붙는다 - 0.55로 맞춰서 실제 필요 도달거리는
# 0.55-0.12=0.43m(실측 자연스러운 도달거리 0.35m 근처), 섀시-받침대 가장자리 간격은
# 0.55-0.175=0.375m로 둘 다 확보한다.
PICK_STANDOFF = 0.55
...
# 사용자 지적: 모든 관절 0도(팔이 완전히 펴진 자세)는 특이점(singularity)에 가까워서, IK가
# 여기서 바로 목표를 풀려고 하면 자코비안이 퇴화돼 불안정/비일관적으로 움직인다(실행마다
# 결과가 들쭉날쭉했던 것도 이 때문일 가능성이 큼). joint_3, joint_5를 90도씩 미리 꺾어
# 특이점에서 벗어난 자세로 시작한 뒤에 IK를 돌린다.
...
print("\n[안정화 완료] 카트/차량/로봇 정지 상태 (joint_3/5 90도로 특이점 회피)\n", flush=True)
...
# 진단 스크린샷으로 확인된 사실: 팔이 이 자세(joint_3/5=90도 시드)에서 자연스럽게 뻗는 실제
# 수평 도달거리는 대략 0.35m 정도인데, 받침대 "중앙"(cart_center)까지는 PICK_STANDOFF(0.70)
# 만큼 떨어져 있어야 했다 - 도달 불가능한 거리였다. 섀시가 받침대 부피 자체와는 여전히
# 안전거리를 두되, 흡착 대상 큐브만 로봇이 다가오는 쪽(+X) 가장자리 근처로 옮겨서 실제
# 필요한 팔 뻗는 거리를 줄인다.
CUBE_EDGE_OFFSET_X = 0.12  # 받침대 중앙 대신 로봇 쪽 가장자리 근처에 놓음
box = DynamicCuboid(
    prim_path="/World/PickBox",
    name="pick_box",
    position=np.array([cart_center_xy[0] + CUBE_EDGE_OFFSET_X, cart_center_xy[1], CART_BASKET_FLOOR_Z + CART_DROP_HEIGHT_ABOVE_FLOOR]),
...
# 진단용: IK를 아직 한 번도 안 돌린, "초기 자세 그대로"인 상태를 먼저 찍어서 초기 자세 자체가
# 카트 쪽을 향하고 있는지, 아니면 애초에 엉뚱한 방향인지부터 확인한다.
init_ee_pos, init_ee_quat = robot.end_effector.get_world_pose()
print(f"[진단] 초기자세(IK 이전) joint_positions={np.round(robot.get_joint_positions(),3)}", flush=True)
print(f"[진단] 초기자세 ee_pos={np.round(init_ee_pos,3)} ee_quat={np.round(init_ee_quat,3)} "
      f"(pick_hover target={np.round(pick_hover_pos,3)})", flush=True)
snapshot(eye=[pick_start_xy[0] - 1.0, pick_start_xy[1] - 1.2, base_pos[2] + 1.0],
         target=[pick_start_xy[0], pick_start_xy[1], base_pos[2] + 0.5], fname="_diag_00_initial_pose.png")

move_link6(pick_hover_pos, steps=200, label="pick_hover")
print(f"[진단] pick_hover 후 joint_positions={np.round(robot.get_joint_positions(),3)}", flush=True)
snapshot(eye=[pick_start_xy[0] - 1.0, pick_start_xy[1] - 1.2, base_pos[2] + 1.0],
         target=[pick_start_xy[0], pick_start_xy[1], base_pos[2] + 0.5], fname="_diag_01_after_hover.png")
...
print("\n[안내] 1단계(PICK+살짝 들어올리기) 검증 완료 - 이제 2단계(주행/회전)로 진행. PLACE는 아직 주석 처리.\n", flush=True)

# ================= 2) TRANSIT: 팔은 그대로 두고(호버 자세 유지) 주행만 =================
print("\n[주행 시작] 카트 -> 트렁크 앞\n", flush=True)
...
turn_in_place(180.0)
box_pos_after_turn, _ = box.get_world_pose()
chassis_pos_after_turn, _ = robot.get_world_pose()
print(f"[회전 후] 로봇 위치=({chassis_pos_after_turn[0]:.3f},{chassis_pos_after_turn[1]:.3f}), "
      f"박스 위치=({box_pos_after_turn[0]:.3f},{box_pos_after_turn[1]:.3f},{box_pos_after_turn[2]:.3f}) "
      f"(흡착 유지={robot.gripper.is_closed()})", flush=True)
snapshot(eye=[chassis_pos_after_turn[0] - 1.0, chassis_pos_after_turn[1] - 1.3, 1.5],
         target=[chassis_pos_after_turn[0], chassis_pos_after_turn[1], base_pos[2] + 0.5], fname="_verify_c2t_02b_after_turn.png")

# ================= 3) 새 위치 기준 RMPflow 베이스 포즈 재보정 =================
base_pos, base_quat, yaw = set_base_pose()

print("\n[안내] 2단계(주행+회전) 검증 완료 - PLACE는 다음 단계에서.\n", flush=True)
simulation_app.close()

# ================= (아래는 다음 단계에서 다시 켤 코드 - 지금은 주석 처리) =================
