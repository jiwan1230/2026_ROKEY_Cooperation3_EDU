...
[카트 박스 목록 바꾸기]
기본값은 Large/Medium/Small 3개 고정 - 다른 개수·크기로 테스트하려면 둘 중 하나:
    --cart-boxes-json '[{"id":"A","width":0.3,"depth":0.2,"height":0.15}, ...]'
    --cart-boxes-file <박스 목록 JSON 파일 경로>  (예시: local_test_data/example_cart_boxes_*.json)

[적재 모드 / 마진 / 쌓기 선택]
loading_mode 파라미터로 "large_first"(기본값, 큰 것부터+입구 접근성 우선)와
"count_first"(최대한 많은 개수 담기)를 고를 수 있다. count_first는 "작은
것부터+공간 재사용 우선" 전략과 "큰 것부터(=large_first와 동일)" 전략을 둘 다
계산해서 더 많이 담기는 쪽을 채택한다(best-of-two) - 박스 크기가 고르게 섞여
있으면 작은 것부터가 오히려 손해일 수 있어서(공간 파편화로 큰 박스가 밀려남),
large_first보다 적게 담는 일이 없도록 08_unloadable_reason.py에서 안전장치를
둔 것. margin 파라미터(미지정 시 -1 = 17_margin_check.MARGIN 기본값 사용)로
벽/박스 최소 간격도 조절 가능 (예: 냉동 물류는 냉기 순환용으로 크게).
allow_stacking(기본 False)을 켜면 1층이 꽉 찼을 때 자동으로 2층·3층에 쌓는다 -
"몇 층까지"를 따로 지정할 필요 없이 바닥에 자리가 있는 동안은 항상 바닥부터
채우고, 꽉 찬 뒤에만 위로 올라간다(⑤ 점수 기준이 원래 낮은 자리 우선이라
자연스럽게 그렇게 동작함). ROS2는 `-p allow_stacking:=true`, --test-file은
`--allow-stacking`.
...
def _save_result_image(
    ...
    mode: str, margin, out_dir: pathlib.Path, allow_stacking: bool = False,
) -> str:
    """
    배치 결과를 local_test_data/_viz_helpers.draw_scene()으로 그려서 PNG로
    저장하고 저장 경로를 반환한다. 파일 이름에 run_id/mode/margin/쌓기허용/
    박스목록 해시를 넣어서, 마진뿐 아니라 카트 박스 목록을 바꿔가며 여러 번
    실행해도 이전 결과 이미지를 안 덮어쓴다 (박스 목록은 --cart-boxes-json/
    --cart-boxes-file로 바뀌는데, 이름이 같은 조합끼리만 겹쳐도 되므로 짧은
    해시로 구분한다 - 전체 JSON을 파일 이름에 넣기엔 너무 길어서).

    2층 이상(z>0)에 놓인 박스는 굵은 점선 테두리로 표시한다(SceneBox의 dashed=
    True - local_test_data의 2층 적재 데모에서 이미 쓰던 관례).
    """
    import hashlib
    ...
    stack_label = "stackon" if allow_stacking else "stackoff"
    boxes_key = json.dumps(cart_boxes_raw, sort_keys=True)
    boxes_hash = hashlib.md5(boxes_key.encode()).hexdigest()[:6]
    out_path = out_dir / f"planner_result_{run_id}_{mode}_margin{margin_label}_{stack_label}_boxes{boxes_hash}.png"

    max_z_layer = max((p.position[2] for p in plans), default=0.0)
    viz.draw_scene(
        ...
        title=f"trunk_map_planner_node 결과 - run={run_id}, mode={mode}, margin={effective_margin:.2f}m, "
              ...
              f"({len(plans)}/{len(plans)+len(unloadable)}개 적재, 최고층 z={max_z_layer:.2f}m)",
...
def _run_test_file(path: str, mode: str, margin, save_image: bool, cart_boxes_raw: list,
    ...
    if save_image:
        out_path = _save_result_image(
            data, trunk, obstacles, cart_boxes_raw, plans, unloadable,
            mode, margin, pathlib.Path(_LOCAL_TEST_DATA_DIR), allow_stacking=allow_stacking,
        )
...
def main():
    ...
    parser.add_argument("--cart-boxes-json",
                         help='카트 박스 목록을 JSON 문자열로 직접 지정 (예: \'[{"id":"A","width":0.3,"depth":0.2,"height":0.15}]\') - 생략하면 기본 3개(Large/Medium/Small)')
    parser.add_argument("--cart-boxes-file",
    ...
    if args.test_file:
        if args.cart_boxes_file:
            cart_boxes_raw = json.loads(pathlib.Path(args.cart_boxes_file).read_text())
        elif args.cart_boxes_json:
