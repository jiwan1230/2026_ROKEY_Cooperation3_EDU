...
def _save_result_image(
    ...
    저장하고 저장 경로를 반환한다. 파일 이름에 run_id/mode/margin/박스목록
    해시를 넣어서, 마진뿐 아니라 카트 박스 목록을 바꿔가며 여러 번 실행해도
    이전 결과 이미지를 안 덮어쓴다 (박스 목록은 --cart-boxes-json/--cart-
    boxes-file로 바뀌는데, 이름이 같은 조합끼리만 겹쳐도 되므로 짧은 해시로
    구분한다 - 전체 JSON을 파일 이름에 넣기엔 너무 길어서).
    """
    import hashlib
    ...
    out_path = out_dir / f"planner_result_{run_id}_{mode}_margin{margin_label}_boxes{boxes_hash}.png"
...
def _run_test_file(path: str, mode: str, margin, save_image: bool, cart_boxes_raw: list) -> None:
