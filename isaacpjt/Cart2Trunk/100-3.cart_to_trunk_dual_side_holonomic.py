...
# 사용자 요청(100-3: main 브랜치의 100.cart_to_trunk_dual_side_holonomic.py를
# 기준으로, cart-stack-more-boxes 브랜치에서 지하주차장 USDZ 환경을 병합) - 기준은
# 무조건 main의 카트/차량/로봇 환경(PICK->PLACE 동작 포함)이고, 주차장은 그 위에
# 배경으로만 얹는다. 99-3/99-2/100-2에서 검증된 것과 완전히 동일한 값/방식 -
# 실측 확인(_view_usdz_env.py로 먼저 단독 로드해서 확인함): src_mpu=0.01,
# src_up=Y, 스케일 적용 후 bbox min=[-14.79,-12.10,-0.11] max=[19.79,11.02,2.89] -
# 카트(x=0)와 차량(x=5) 둘 다 이 X 범위 안에 넉넉히 들어간다. 실제 주차장 바닥이
# 기존 환경의 바닥(Z=0)보다 약 0.112m 아래에 있으므로, 배치 시 그만큼 위로
# 올려서 두 바닥을 맞춘다.
...
def add_static_collision(stage, root_prim_path, exclude_ancestor_keywords=("plane",)):
    """사용자 실측 확인(2026-07-27) - 지하주차장에 콜리전이 전혀 없어서 카트/차량
    바퀴가 기본 지면(Z=0, 평평한 가상 평면)에만 얹혀있었고, 주차장 바닥 실제 메시와
    미세하게 안 맞아 바퀴가 살짝 파묻힌 것처럼 보였다. add_sdf_collision()은 SDF
    근사(움직이는 물체와 정확히 맞물려야 할 때 필요, 코킹 비용 큼)를 쓰는데,
    주차장은 RigidBodyAPI가 없는 완전히 정적인 배경이라 그럴 필요가 없다 -
    PhysX는 RigidBodyAPI 없는 콜리전 메시를 자동으로 정적(static) 콜라이더로
    취급하고, 정적 콜라이더는 approximation="none"(원본 삼각망 그대로)이 SDF보다
    훨씬 가볍게 코킹된다(실측: 14.9MB짜리 메시 77개에 0.02초). 이러면 카트/차량이
    주차장의 실제 바닥 형상(턱/굴곡 포함) 위에 정확히 얹힌다.

    사용자 실측 확인(2026-07-27, 2차) - 콜리전을 켰더니 이번엔 주차선(Plane_001_6,
    Plane_46, Plane_002_47, Plane_003_48, Plane_004_49, Plane_005_50 - 전부 두께
    ~1cm짜리 얇고 넓은 데칼 메시로 확인됨, GARAGE_USD 안에서 이름에 "Plane"이 들어간
    프림은 이 6개가 전부)에 로봇이 걸려 이동을 못 하는 문제가 생겼다. 이 얇은 바닥
    데칼들은 실제 지면과 거의 같은 높이에 겹쳐 있어서 콜리전이 있으면 얇은 턱처럼
    작용해 바퀴가 걸린다 - 조상 프림 이름에 "plane"이 들어간 메시는 콜리전에서
    제외한다(바닥 자체 메시는 별도로 존재하므로 주차선 데칼 콜리전이 없어도 바닥
    지지력에는 영향 없음)."""
    root_prim = stage.GetPrimAtPath(root_prim_path)
    n, skipped = 0, 0
    for prim in Usd.PrimRange(root_prim):
        if prim.GetTypeName() == "Mesh":
            _ancestor = prim.GetParent()
            _excluded = False
            while _ancestor and _ancestor.IsValid() and _ancestor != root_prim.GetParent():
                _aname = _ancestor.GetName().lower()
                if any(kw in _aname for kw in exclude_ancestor_keywords):
                    _excluded = True
                    break
            ...
            n += 1
    print(f"[정적 콜리전] {root_prim_path}: {n} mesh (주차선 등 {skipped}개 제외)", flush=True)
...
def match_physical_prim(stage, scan_center_world, available_paths):
    # 실측 확인(2026-07-27, 100-3) - main 브랜치 박스 배치는 자식(Medium/Small)을
    # 각자의 부모 Large "정중앙(centered)"에 그대로 얹는다(검출 안정성을 위한 의도적
    # 설계 - CART_BOX_SPECS 정의부 주석 참고). 그 결과 자식과 부모의 XY 중심이
    # 거의 같아져서, XY만 보고 가장 가까운 프림을 고르던 기존 방식은 Medium의
    # 검출 결과를 Large1 물리 프림에 잘못 매칭시켰다(실측: 매칭거리는 5mm로
    # 작았지만 실제로는 서로 다른 높이의 박스였음) - 이 때문에 PICK이 Medium 높이
    # 기준으로 계산된 목표를 향해 Large1을 집으려다 흡착 실패로 중단됐다. Z까지
    # 포함한 3D 거리로 바꾸면 스택된 자식/부모처럼 XY가 겹쳐도 높이 차이로 확실히
    # 구분된다(원래 하드코딩된 in-cart 배치에서 이 정도 층 간격은 항상 XY 매칭
    # 오차보다 훨씬 크다).
...
# 사용자 요청(100-3) - 지하주차장 배경을 차량/카트/로봇보다 먼저 추가한다(순서
# 자체는 중요하지 않음 - 서로 다른 prim path라 간섭 없음).
...
# 사용자 실측 확인(2026-07-27) - 카트/차량 바퀴가 주차장 바닥에 살짝 파묻혀 보임(콜리전
# 없어서 기본 평면 지면에만 얹혀있었음). add_static_collision() 정의부 설명 참고 -
# 정적 배경이라 SDF 대신 가벼운 "none"(원본 삼각망) 콜리전으로 충분하다(실측 0.02초).
add_static_collision(stage, "/World/Garage")
...
# 사용자 요청(2026-07-27, 3차) - 3배(2.4x1.34m)로도 입구 구멍을 다 못 가려서
# "더 크게, 옆의 구멍을 아예 막아버릴 생각으로" 키워달라고 함. map_doorway.py
# 측정 기준: 벽면(Y=4.888) X=0~1.7m 구간이 열려 있고, 높이 방향은 z=0.25(문턱
# 근처) ~ z=2.0(위쪽에서 벽이 다시 시작) 범위가 대부분 뚫려 있음. 5배 확대
# (0.8x0.447 -> 4.0x2.235m)하고, 중심을 X=0.85(입구 폭 중앙), Z=1.2(바닥~약
# 2.3m를 덮도록 살짝 낮춤)로 놓아 입구 폭(1.7m)과 높이(~2m) 모두 넉넉히 덮는다.
SIGN_USD = str(_THIS_DIR / "assets/SignRokeyMart.usd")
SIGN_BIG_SCALE = 5.0
SIGN_POS = (0.85, 4.86, 1.2)
add_asset(stage, "/World/SignRokeyMart", SIGN_USD, Gf.Vec3d(*SIGN_POS), SIGN_BIG_SCALE, target_mpu, target_up)
print(f"[간판] 로키마트 간판(대형, x{SIGN_BIG_SCALE}) 설치 완료: {SIGN_POS}", flush=True)
...
for _box_num, (picked_prim_path, picked_placement) in enumerate(pick_order):
    ...
    # 실측 확인(2026-07-27, 100-3, 사용자 GUI 관찰 + 진단 로그) - move_link6_smooth()는
    # 매 스텝 "위치(조금씩 목표로 접근)"와 "자세(처음부터 곧장 DOWN_QUAT)"를 동시에
    # RMPflow에 요구한다. 접힌 자세(joint_3/5=±90도)의 실제 그리퍼 방향은 아래를 보고
    # 있지 않으므로(팔꿈치만 굽힌 자세라 옆/앞을 봄), 호버 이동은 "수평 이동"과
    # "아래로 재정렬"을 동시에 풀어야 하는데 - 표준 접힘(+90도) 시드에서는 이 둘을
    # 동시에 풀어도 우연히 팔꿈치가 굽은 채로 수렴하지만, 반전 접힘(-90도) 시드에서는
    # 같은 방식으로 풀다가 팔꿈치가 완전히 펴지는 쪽(0도)으로 넘어가버렸다(진단 로그로
    # 확인). 위치는 거의 안 바꾸고 자세만 먼저 DOWN_QUAT으로 맞추는 중간 단계를 넣어서
    # "재정렬"과 "수평 이동"을 분리한다 - 재정렬을 마친 뒤에는 방향이 이미 고정돼
    # 있으니 이후 호버 이동은 순수 위치 문제가 되어 RMPflow가 국소해를 잘못 고를
    # 여지가 줄어든다(관절을 직접 건드리지 않는 방식이라 이전 두 시도보다 안전함).
    _pre_hover_ee_pos, _ = m0609_robot.end_effector.get_world_pose()
    move_link6(_pre_hover_ee_pos, steps=150, hold_gripper_closed=False, orientation=DOWN_QUAT,
               label="호버 전 자세 정렬(위치 고정, 방향만 정렬)")
    ...
    # 실측 확인(2026-07-27, 100-3) - 호버는 재정렬 웨이포인트로 고쳤지만(위 참고), 하강은
    # XY/방향 다 그대로고 순수 Z만 내려가는데도 반전 접힘에서 여전히 발산했다(err
    # 0.20m->0.63m) - 카트에 가까워질수록 반전 쪽에서만 팔이 펴지는 움직임이 나오는
    # 것으로 보인다(사용자가 GUI에서 본 "카트 외벽에 부딪힘"과 일치). 한 번에 먼 목표까지
    # 계속 당기는 대신, Z를 몇 단계로 나눠 중간 지점마다 한 번씩 새로 목표를 잡게 한다 -
    # TILT-4 발산을 고친 것과 같은 원리(작은 목표를 여러 번 vs 큰 목표를 한 번).
    # 실측 확인(2026-07-27) - 4단계에서는 1~3/4(카트에서 먼 구간)은 전부 깨끗이
    # 수렴했는데 마지막 4/4(박스에 가장 가까운 구간)에서만 발산했다(err 0.42m) -
    # 문제가 "카트/박스에 가까워질수록"에 집중된다는 뜻이므로, 단계를 늘려 마지막
    # 접근 구간을 더 잘게 쪼갠다.
    _descent_substeps = 8
    _descent_start_z = float(hover_target[2])
    for _ds_i in range(1, _descent_substeps + 1):
        if m0609_robot.gripper.is_closed():
            break
        _ds_alpha = _ds_i / _descent_substeps
        _ds_z = _descent_start_z + (float(descent_target[2]) - _descent_start_z) * _ds_alpha
        _ds_target = np.array([descent_target[0], descent_target[1], _ds_z])
        move_link6_smooth(_ds_target, max_speed=DESCENT_MAX_SPEED, try_grasp=True,
                           label=f"하강 {_ds_i}/{_descent_substeps}")
    ...
    if STAGE >= 2:
        ...
        def tilt_and_insert_through_entrance(entrance_x, box_dims, tilt_deg=None, approach_standoff=None,
            ...
            # 실측 확인(2026-07-27, 100-3, Large1) - tilt_deg가 겨우 2도라 6단계로 나눠도
            # 한 단계당 0.33도밖에 안 되는데도 첫 단계부터 err=0.575m로 발산했다(목표
            # target=[3.54,-0.081,...] vs 실제 ee=[3.252,-0.539,...] - Y축으로만 0.46m
            # 벗어남). 각도 스텝을 더 잘게 쪼개는 것만으론 (이미 충분히 잘게 쪼갰으므로)
            # 부족해 보여 - 한 단계당 스텝 수를 늘려 RMPflow가 매 목표에 더 오래 수렴할
            # 시간을 준다(기존 max(20,...)는 tilt_steps=150 기준 25스텝뿐이었음).
            RESTORE_SUBSTEPS = 6
            ...
            for _sub_i in range(1, RESTORE_SUBSTEPS + 1):
                ...
                restore_ee, restore_err = move_link6(
                    restore_pos, steps=max(60, tilt_steps // RESTORE_SUBSTEPS), hold_gripper_closed=True,
