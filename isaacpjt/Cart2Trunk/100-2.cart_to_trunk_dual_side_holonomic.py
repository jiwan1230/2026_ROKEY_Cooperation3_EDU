...
# 사용자 요청(100-2: 지하주차장 USDZ 환경에 100번의 카트+로봇+박스+차량(트렁크)
# 환경을 병합) - 기준은 무조건 기존 100번 환경(카트/차량/로봇 위치, 물리, PICK
# ->PLACE 동작)이고, 주차장은 그 위에 배경으로만 얹는다. 99-2.cart_scan_dual_side_
# holonomic.py에서 검증된 것과 완전히 동일한 값/방식 - 실측 확인(_view_usdz_env.py
# 로 먼저 단독 로드해서 확인함): src_mpu=0.01, src_up=Y, 스케일 적용 후 bbox
# min=[-14.79,-12.10,-0.11] max=[19.79,11.02,2.89] - 카트(x=0)와 차량(x=5) 둘 다
# 이 X 범위 안에 넉넉히 들어간다. 실제 주차장 바닥이 우리 기존 환경의 바닥(Z=0,
# 카트/차량/로봇이 이미 그 기준으로 검증됨)보다 약 0.112m 아래에 있으므로, 배치
# 시 그만큼 위로 올려서 두 바닥을 맞춘다.
...
# 사용자 요청(100-2) - 지하주차장 배경을 차량/카트/로봇보다 먼저 추가한다(순서
# 자체는 중요하지 않음 - 서로 다른 prim path라 간섭 없음). 99-2.cart_scan_dual_
# side_holonomic.py와 동일한 이유로 순수 배경(충돌 없음)으로만 쓴다 - 이 에셋은
# 14.9MB짜리 복잡한 메시라 add_sdf_collision()으로 전체 메시에 SDF 콜리전을
# 걸면 코킹이 오래 걸리거나 실패할 위험이 있고, 로봇/카트/차량은 어차피 기존에
# 검증된 기본 지면(add_default_ground_plane)과 카트/트렁크 위에서만 동작하지
# 주차장 구조물(기둥/벽 등)과 부딪힐 일이 없는 열린 바닥에 놓을 것이므로 콜리전이
# 굳이 필요 없다 - "기존 환경을 기준으로 에러 없이 병합"이라는 요구에 맞춰 위험을
# 최소화하는 선택.
...
# 실측 확인(사용자 지적: Medium/Small이 Large 가장자리 밖으로 삐져나옴) - 기존
# offset(0.045, ±0.075/0.085)은 원래 Large(0.30x0.22) 기준으로 고른 절대값이라,
# 그걸 그대로 비율만 축소하면 상대적인 "가장자리에서 얼마나 튀어나오는가" 비율도
# 그대로 유지된다 - 실은 원래 3박스 설계도 폭(y)축으로 1.5cm 정도 튀어나와 있었다
# (Large half_y=0.11, Medium offset=-0.075, half=0.05 -> -0.125가 -0.11 밖).
# dx(길이축)는 이미 여유가 있어 그대로 두고, dy(폭축)만 "Large 절반 폭 - 자식
# 절반 폭 - 안전마진"으로 매번 다시 계산해서 어떤 LARGE_SIZE에서도 가장자리를
# 넘지 않게 한다 - 그러면서도 한쪽 끝까지 최대한 붙여서(값을 0으로 만들지 않음)
# 반대쪽에 검출용 노출 스트립을 최대한 남기는 기존 설계 의도는 유지한다.
...
CART_BOX_SPECS = [
    ...
    ("Medium", MEDIUM_SIZE, (-_cart_large_dx + 0.045 * _large_scale_x, _medium_dy), 0.6),
    ("Small", SMALL_SIZE, (_cart_large_dx + 0.045 * _large_scale_x, _small_dy), 0.3),
]
