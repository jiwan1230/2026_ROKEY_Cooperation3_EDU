...
실제로 다른 결과(적재 개수)를 내는 걸 보여준다 - large_first보다 count_first가
더 많이 담는다는 부등식은 회귀 테스트로 고정돼 있지만, 정확한 개수는 ③ 후보
생성기가 바뀔 때마다 greedy 선택 경로가 살짝 달라져 바뀔 수 있다(예: 입구쪽
벽 플러시 후보 추가로 4/8 vs 7/8이 4/8 vs 6/8이 됨) - 실행 시 실제 숫자를
콘솔 출력으로 확인.
"""
import sys
import pathlib
