...
def test_floor_candidate_always_fully_supported():
    ...
    x/y는 벽 안전 여유(MARGIN) 이상이어야 하므로 0.0 대신 MARGIN을 쓴다 - z(바닥)는
    여유가 필요 없어 그대로 0.0."""
    trunk = Trunk(width=1.0, depth=1.0, height=1.0)
    box = Box("A", width=0.4, depth=0.3, height=0.2)

    assert compute_support_ratio(MARGIN, MARGIN, 0.0, box, placed=[]) == 1.0
    assert is_candidate_valid_with_stacking(
        MARGIN, MARGIN, 0.0, box, trunk, placed=[], allow_stacking=False
    ) is True


def test_fully_supported_box_on_top_is_valid_when_stacking_allowed():
    """아래 박스와 같은 크기의 박스를 정확히 그 위에 놓으면 받침 비율 1.0 -> 유효.
    x/y는 벽 안전 여유만큼 띄우고(below/on_top 둘 다 같은 x,y라 스택 관계는 그대로
    유지), z(쌓는 방향)는 여유 없이 딱 맞닿아야 한다."""
    trunk = Trunk(width=1.0, depth=1.0, height=1.0)
    below = PlacedBox(box=Box("Base", width=0.4, depth=0.4, height=0.2), x=MARGIN, y=MARGIN, z=0.0)
    on_top = Box("Top", width=0.4, depth=0.4, height=0.2)

    ratio = compute_support_ratio(MARGIN, MARGIN, 0.2, on_top, placed=[below])
    assert ratio == 1.0
    assert is_candidate_valid_with_stacking(
        MARGIN, MARGIN, 0.2, on_top, trunk, placed=[below], allow_stacking=True
    ) is True
...
def test_small_base_rejects_much_larger_box_on_top():
    ...
    x/y를 벽 안전 여유(MARGIN)만큼 띄운다 - 0.0을 그대로 쓰면 벽 여유 위반으로도
    거부되어서(False), 이 테스트가 실제로 검증하려는 "받침 비율 부족" 경로를 타지
    않고도 우연히 통과해버릴 수 있다.
    """
    trunk = Trunk(width=1.0, depth=1.0, height=1.0)
    small_base = PlacedBox(box=Box("Small", width=0.2, depth=0.2, height=0.3), x=MARGIN, y=MARGIN, z=0.0)
    big_top = Box("Big", width=0.6, depth=0.6, height=0.2)

    ratio = compute_support_ratio(MARGIN, MARGIN, 0.3, big_top, placed=[small_base])
    assert ratio < 0.8

    assert is_candidate_valid_with_stacking(
        MARGIN, MARGIN, 0.3, big_top, trunk, placed=[small_base], allow_stacking=True
    ) is False
...
def test_combined_support_from_two_adjacent_boxes():
    ...
    left/right는 서로 딱 붙어있는 관계 자체를 검증하는 게 목적이라(is_candidate_valid를
    거치지 않고 직접 구성) 여유 없이 맞닿게 두고, on_top 후보의 x/y만 벽 안전 여유를
    반영한다.
    """
    trunk = Trunk(width=1.0, depth=1.0, height=1.0)
    left = PlacedBox(box=Box("Left", width=0.25, depth=0.4, height=0.2), x=MARGIN, y=MARGIN, z=0.0)
    right = PlacedBox(box=Box("Right", width=0.25, depth=0.4, height=0.2), x=MARGIN + 0.25, y=MARGIN, z=0.0)
    on_top = Box("Top", width=0.5, depth=0.4, height=0.2)

    ratio_left_only = compute_support_ratio(MARGIN, MARGIN, 0.2, on_top, placed=[left])
    assert abs(ratio_left_only - 0.5) < 1e-9  # 하나만 있으면 절반만 지지

    ratio_combined = compute_support_ratio(MARGIN, MARGIN, 0.2, on_top, placed=[left, right])
    assert ratio_combined == 1.0

    assert is_candidate_valid_with_stacking(
        MARGIN, MARGIN, 0.2, on_top, trunk, placed=[left, right], allow_stacking=True
    ) is True
...
def test_allow_stacking_false_rejects_even_fully_supported_candidate():
    ...
    x/y를 벽 안전 여유만큼 띄운다 - 이유는 test_small_base_rejects_much_larger_box_on_top
    참고(벽 여유 위반이 아니라 allow_stacking 자체의 거부 경로를 실제로 타는지 확인)."""
    trunk = Trunk(width=1.0, depth=1.0, height=1.0)
    below = PlacedBox(box=Box("Base", width=0.4, depth=0.4, height=0.2), x=MARGIN, y=MARGIN, z=0.0)
    on_top = Box("Top", width=0.4, depth=0.4, height=0.2)

    assert is_candidate_valid_with_stacking(
        MARGIN, MARGIN, 0.2, on_top, trunk, placed=[below], allow_stacking=False
    ) is False
