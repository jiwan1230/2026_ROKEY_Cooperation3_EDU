...
## PART 2-A(Space / Loading Algorithm, 담당: 선욱) 범위 대조표

팀 시스템 아키텍처 다이어그램의 PART 2-A 담당 항목 4개를 실제로 어디까지
구현했는지 대조한 표. "전부 했다"가 아니라 **어디서부터 어디까지가 이 모듈의
경계인지**를 명확히 하는 게 목적 (다른 파트와의 데이터 계약을 맞추려면 필요).

| 항목 | 한 것 | 안 한 것 / 경계 |
|---|---|---|
| **Trunk Point Cloud 처리** | `02_trunk_space_state.py`가 이미 처리된 `trunk_map.json`(코너 좌표 등)을 받아 `Trunk`/장애물 객체로 변환 | 원본 포인트 클라우드(점 수만 개)를 직접 클러스터링/세그멘테이션하는 건 안 함 — PART1(비전)이 그 결과를 넘겨준다는 전제 |
| **Occupancy / Free Space Map** | `ExtremePointState`(놓인 박스 + 후보 좌표 집합)로 기능적으로 동일한 역할 수행 | `TrunkOccupancyMap`이라는 이름의 정식 결과물(예: 격자 형태)로 내보내는 변환기는 아직 없음 - 공통 데이터 계약과 맞추려면 어댑터 필요 |
| **적재 가능성 · 순서 결정** | ⑥(픽업 순서, `rests_on_id` 위상정렬) + ⑦(배치) + ⑧(가능 여부 + 사유 3종 + 재배치 판단) - 이 프로젝트에서 가장 깊게 다룬 부분 | - |
| **Loading Pose / 방향 생성** | 위치(x,y,z) + 회전 여부(⑱, 0°/90° 중 선택)까지 결정 | 완전한 3D 방향(기울임 등)이나 로봇 표준 Pose 포맷(quaternion 등)으로 출력하진 않음 - 지금은 `rotated` bool + 치수로만 표현 |

**다이어그램에 안 적혀 있지만 추가로 만든 것** (적재 가능성 판단의 하위 안전
규칙들 + 엔지니어링 인프라):

- ⑬ 받침 확인, ⑮ 상단 여유 공간, ⑯ 접근 경로 확인, ⑰ 박스-벽/박스 마진(1cm) - 전부 "적재 가능성" 판단을 더 정교하게 만드는 하드 컷 규칙들
- ⑨ 재스캔 후 재계획 (`09_rescan_replan.py`)
- ⑭ 실제 비전 데이터로 돌리는 CLI 진입점 (`14_run_full_pipeline.py`)
- ⑲ 무작위 스트레스 테스트 (`19_random_stress_test.py`) - 여러 시드로 1,700+ 무작위 시행 검증 (전부 통과)
- pytest 67케이스 + 검증 스크립트 2개(`10_verification.py`, `12_verify_real_coords.py`) - 회귀 안전장치
- 인터랙티브 3D 좌표 편집기 (`local_test_data/interactive_cart_reload_editor.py`) - 개발용 도구
