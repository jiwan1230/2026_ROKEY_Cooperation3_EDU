"""
99.cart_scan_dual_side_holonomic.py

88.cart_scan_holonomic.py를 100.cart_to_trunk_dual_side_holonomic.py의 "카트 90도
회전" 재설계에 맞게 재구현(사용자 지시, 2026-07-26) - 100번이 카트를 회전시키면서
카트+박스 배치가 바뀌었는데(CART_ROT_Z=90도, CART_BOX_SPECS 오프셋이 Y축->X축), 88번이
남긴 스캔 결과(perception/base_to_camera_transform.json, scan_cache/merged_cart_scan.npy
-> ~/box_pointcloud/all_boxes_corners_*.json)는 전부 회전 전 카트 기준이라 100번을
실제로 돌리면 PICK이 완전히 엉뚱한 위치를 잡으러 간다(사용자 실측 확인 - hover/descent
둘 다 800~1000스텝 정체 후 흡착 실패, 스캔이 준 좌표(Y=-0.155 근방)가 실제 박스
위치(X=-0.157 근방, 회전으로 축이 뒤바뀜)와 전혀 다른 곳을 가리키고 있었다).

88번과 동일하게 유지되는 부분 - 다중 시점 스캔/ROI 필터/point cloud 저장/ROS2 카메라
브리지 전체 로직, run_scan_batch.py로 넘기는 파이프라인(box_top_extractor.py 관련
레거시 경로 포함)은 문자 그대로 재사용한다. ROI 필터(cart_min/max ± margin)는
cart_min/max를 회전 후 실제 bbox에서 매번 새로 재므로 코드 변경 없이 자동으로
맞는 범위를 쓴다.

100번과 동일하게 맞춘 부분(구조적으로 달라진 곳)
----
1) CART_ROT_Z=90도 - 100번과 완전히 동일한 값/이유(카트 입구가 손잡이 반대쪽,
   차량 쪽을 보도록). 100번과 다른 각도로 스캔하면 스캔 결과와 실제 PICK 씬의 카트
   방향이 어긋나므로 반드시 100번과 같은 값을 써야 한다 - 100번에서 부호를 뒤집으면
   여기도 그대로 따라 뒤집어야 한다.
2) CART_BOX_SPECS 오프셋 - 100번과 완전히 동일하게 Y축(옛 길이축)에서 X축(회전 후
   길이축)으로 옮겼다(Box_A는 -X/손잡이쪽, Box_B는 +X/입구쪽).
3) 카트 옆 standoff/스캔 위치 - 88번은 카트의 "폭"(옛 X)만큼 떨어져 서서 카트의
   "길이"(옛 Y)를 따라 strafe(평행이동)하며 5개 시점을 스캔했다. 회전 후엔 폭이 Y,
   길이가 X이므로, standoff를 Y 오프셋으로 서고 strafe를 X 방향으로 한다(스캔
   시야/ROI 로직 자체는 축 이름만 바꿔 그대로).
4) 스폰 위치 안전 - 100번에서 실측된 함정과 완전히 동일 - BASE_FACE_ROT_Z=90도가
   고정이라 섀시는 항상 yaw=90도로 스폰되는데, 이 각도에서는 섀시의 "긴" 길이 축이
   Y를 향한다. standoff가 Y 오프셋인 채로 그 자리에 그냥 스폰하면 스폰 즉시 카트와
   겹친다 - 100번과 동일한 CART_CLEAR_X(카트 길이축 연장선상의 회전 안전지대) +
   순차 접근(회전 없이 안전지대로 -> 안전지대에서 회전+횡이동 -> 최종 X 접근)을
   그대로 적용한다(88번의 더 단순한 drive_to에 맞춰, abort_fn 없이 순서만 나눴다 -
   88번은 애초에 박스를 들고 있지 않아 100번만큼 충돌 감지가 절실하지 않다).

이 스크립트가 다루지 않는 것 - 88번과 동일(box_top_extractor.py 결과 JSON 변환은
run_scan_batch.py가 이미 담당, 트렁크 스캔/PICK/PLACE는 100번 참고).
"""
...
# 사용자 요청(2026-07-27) - 실제 주차 구획 자리(이전에 (-0.243,-2.727)로 계산해
# 확인했던 위치, 카메라 intrinsic 실측 기반 픽셀->월드 변환 방법은 검증됨)에
# 배치하는 작업은 나중에 다시 하기로 하고, 일단은 원래 기본값(주차장 원점)으로
# 되돌린다.
CART_POS = (0.0, 0.0, 0.0)
CART_EXTRA_SCALE = 0.55

# 사용자 요청(99-2: 지하주차장 USDZ 환경에 지금까지 검증된 카트+로봇+박스 환경을
# 병합) - 기준은 무조건 기존 환경(카트/로봇/박스 위치, 물리)이고, 주차장은
# 그 위에 배경으로만 얹는다. 실측 확인(_view_usdz_env.py로 먼저 단독 로드해서
# 확인함): src_mpu=0.01, src_up=Y, 스케일 적용 후 bbox min=[-14.79,-12.10,-0.11]
# max=[19.79,11.02,2.89] - 실제 주차장 바닥이 우리 기존 환경의 바닥(Z=0, 카트/
# 로봇이 이미 그 기준으로 검증됨)보다 약 0.112m 아래에 있으므로, 배치 시 그만큼
# 위로 올려서 두 바닥을 맞춘다.
GARAGE_USD = "/home/rokey/Downloads/Basement_Parking_Garage_Environment_Underground.usdz"
GARAGE_EXTRA_SCALE = 1.0
GARAGE_FLOOR_Z_OFFSET = 0.112
GARAGE_POS = (0.0, 0.0, GARAGE_FLOOR_Z_OFFSET)
# 100.cart_to_trunk_dual_side_holonomic.py와 반드시 동일한 값이어야 한다(사용자 지시) -
# 스캔 결과가 가리키는 카트 방향과 실제 PICK 씬의 카트 방향이 어긋나면 안 되므로. 100번의
# CART_ROT_Z 정의부 참고 - 손잡이(A단)가 차량에서 먼 쪽(-X), 입구(B단)가 가까운
# 쪽(+X)이 되도록 하는 부호다. 100번에서 부호를 뒤집었다면 여기도 그대로 따라 뒤집는다.
CART_ROT_Z = 90.0
SDF_RESOLUTION = 256
...
# 100.cart_to_trunk_dual_side_holonomic.py와 동일 - CART_CLEAR_X(회전 안전지대) 계산에
# 필요한 섀시 "긴" 길이 축의 유효 반경(88번엔 원래 없었다 - 88번은 폭 축만 카트 쪽으로
# 향하는 단일 standoff만 썼기 때문).
...
# 사용자 요청(99-2) - 지하주차장 배경을 카트/로봇보다 먼저 추가한다(순서
# 자체는 중요하지 않음 - 서로 다른 prim path라 간섭 없음). 순수 배경(충돌 없음)
# 으로만 쓴다 - 이 에셋은 14.9MB짜리 복잡한 메시라 add_sdf_collision()으로 전체
# 메시에 SDF 콜리전을 걸면 코킹이 오래 걸리거나 실패할 위험이 있고, 로봇/카트는
# 어차피 기존에 검증된 기본 지면(add_default_ground_plane)과 카트 위에서만
# 동작하지 주차장 구조물(기둥/벽/주차된 차량 등)과 부딪힐 일이 없는 열린 바닥에
# 놓을 것이므로 콜리전이 굳이 필요 없다 - "기존 환경을 기준으로 에러 없이
# 병합"이라는 요구에 맞춰 위험을 최소화하는 선택.
add_asset(stage, "/World/Garage", GARAGE_USD, GARAGE_POS, GARAGE_EXTRA_SCALE, target_mpu, target_up)
for _ in range(30):
    simulation_app.update()
print(f"[배경] 지하주차장 로드 완료: {GARAGE_USD}", flush=True)
...
# 90도 회전 후 cart_half_x는 카트의 "길이"(A-B, 손잡이-입구) 반쪽 폭, cart_half_y는
# "너비"(standoff가 갈라지는 축) 반쪽 폭이다(100번과 동일 - bbox_of()가 회전 후 실제
# 월드 AABB에서 매번 새로 재므로 하드코딩 없이 자동으로 뒤바뀐다).
...
# 사용자 요청(2026-07-27: 일단 박스는 원래대로 4개로 되돌리고, 로봇/카트 위치
# 튜닝은 나중에 다시 하기로 함) - 99-1/99-2에서 실험했던 5개(Large 2개 +
# Medium1/Medium2/Small) 구조는 잠시 접어두고, 99번/100번과 동일한 4개 구조
# (Large 2개, 각각 자식 1개씩 - Large1 위 Medium, Large2 위 Small)로 되돌린다.
# 트렁크 크기 제약(모든 변 0.21m 미만)과 "자식이 부모 가장자리 밖으로 삐져나오면
# 안 된다"는 조건은 그대로 유지한다.
...
_EDGE_SAFETY_MARGIN_M = 0.01

# Large1/Large2를 99/100번과 동일하게 카트 바닥에 나란히 놓는다(간격 6cm -
# 88번에서 실측 확인된, 형제 박스 착지 충돌을 피하는 안전 마진).
CART_LARGE_GAP_M = 0.06
_cart_large_dx = (LARGE_SIZE[0] + CART_LARGE_GAP_M) / 2.0

# Medium/Small 각각 자식이 하나뿐이라(부모당 1개) 99/100번과 동일하게 "Large
# 절반 폭 - 자식 절반 폭 - 안전마진"만큼 한쪽 끝으로 밀어서 반대쪽에 노출
# 스트립을 남긴다. 여기서부터는 자식 offset이 "카트 중심"이 아니라 "부모(Large)
# 중심" 기준 로컬 offset이다(아래 단계별 스폰 루프가 부모 실측 위치 + 이 로컬
# offset으로 자식 스폰 좌표를 계산한다).
_medium_dy = -(LARGE_SIZE[1] / 2.0 - MEDIUM_SIZE[1] / 2.0 - _EDGE_SAFETY_MARGIN_M)
_small_dy = (LARGE_SIZE[1] / 2.0 - SMALL_SIZE[1] / 2.0 - _EDGE_SAFETY_MARGIN_M)
_medium_local = (0.045 * _large_scale_x, _medium_dy)
_small_local = (0.045 * _large_scale_x, _small_dy)

CART_STACK_BASE_NAMES = ["Large1", "Large2"]
_STACK_PARENT = {"Medium": "Large1", "Small": "Large2"}
CART_BOX_SPECS = [
    # (name, size(x,y,z), 부모(자식이면) 또는 카트(바닥이면) 중심 기준 offset(dx=길이축, dy=폭축), mass_kg)
    ("Large1", LARGE_SIZE, (-_cart_large_dx, 0.0), 1.2),
    ("Large2", LARGE_SIZE, (_cart_large_dx, 0.0), 1.2),
    ("Medium", MEDIUM_SIZE, _medium_local, 0.6),
    ("Small", SMALL_SIZE, _small_local, 0.3),
]
...
# 실측 확인(사용자 지적: Medium2/Small이 Large2 위가 아니라 완전히 다른 곳에
# 떨어짐, 88.cart_scan_holonomic.py에서 이미 겪고 고친 것과 같은 버그) - 부모와
# 자식이 동시에 자유낙하하다가 부모가 먼저 멈추면 자식은 남은 낙차만큼만
# 떨어져 안착한다는 트릭은 "부모가 착지 중 안 밀린다"는 가정이 있어야 성립
# 하는데, Large2처럼 자식을 2개(Medium2+Small)나 한쪽 끝에 얹은 무거운 배치는
# 착지 충격에 밀리기 쉽다 - 자식은 이미 스폰 시점에 고정된 XY로 계속 떨어지므로
# 부모가 밀리면 완전히 빗나간다(실측: Medium2가 Large2 옆 17cm, Small은 Large2
# 자체보다 낮은 높이까지 떨어짐). 그래서 부모(Large1/Large2)를 먼저 스폰+실제
# 물리로 정착시키고 그 실측 위치를 읽어와 자식(Medium1/Medium2/Small)의 스폰
# 좌표로 쓴다 - 부모가 얼마나 밀리든 자식은 항상 부모의 "진짜" 최종 위치 위에서
# 시작한다. (여기서부터는 step_hold()가 이미 정의돼 있어야 하므로, 실제 스폰은
# 뒤쪽 "카트 안 적층 박스 스폰+정착" 지점에서 진행한다 - CART_BOX_SPECS 정의는
# 여기서 끝나고, DynamicCuboid 생성은 뒤로 미룬다.)
...
# 카트 안 적층 박스 레벨별 순차 스폰+정착(88.cart_scan_holonomic.py와 동일 원리 -
# CART_BOX_SPECS 정의부의 설명 참고). 레벨 0 = 바닥(Large1/Large2), 레벨 1 = 그
# 위 자식(Medium/Small) - 이 시나리오는 2단이라 레벨이 항상 이 둘뿐이다.
_cart_box_xy_by_name = {}
_cart_box_top_z_by_name = {}
_cart_box_size_by_name = {}
_cart_box_levels = [
    [t for t in CART_BOX_SPECS if t[0] in CART_STACK_BASE_NAMES],
    [t for t in CART_BOX_SPECS if t[0] not in CART_STACK_BASE_NAMES],
]
for _level_idx, _level in enumerate(_cart_box_levels):
    for name, size, (dx, dy), mass_kg in _level:
        if name in CART_STACK_BASE_NAMES:
            ...
            parent_name = _STACK_PARENT[name]
    ...
    for name, size, _off, _m in _level:
...
if os.environ.get("CART2TRUNK_GT_DEBUG", "0") == "1":
    ...
    for _gte_name, _gte_size, _gte_off, _gte_mass in CART_BOX_SPECS:
        _gte_prim = stage.GetPrimAtPath(f"/World/Box_{_gte_name}")
        _gte_world = np.array(_gte_prim.GetAttribute("xformOp:translate").Get())
        print(f"[GT_DEBUG early] {_gte_name}: world={_gte_world.tolist()}", flush=True)
...
if os.environ.get("CART2TRUNK_GT_DEBUG", "0") == "1":
    ...
    for _gt_name, _gt_size, _gt_off, _gt_mass in CART_BOX_SPECS:
        ...
        print(f"[GT_DEBUG] {_gt_name}: base_frame_center={_gt_base.tolist()} "
              f"top_z={_gt_base[2] + _gt_size[2] / 2.0:.4f} size={_gt_size}", flush=True)
