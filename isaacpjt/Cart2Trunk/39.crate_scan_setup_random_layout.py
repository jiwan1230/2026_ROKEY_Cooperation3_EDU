"""
39.crate_scan_setup_random_layout.py
38.crate_scan_setup_random_boxes.py의 복사본 - 35.py/38.py는 그대로 두고(둘 다 잘
동작하는 버전이라 보존), 여기서는 38.py의 크기 랜덤화(TABLE_BOX_BASE_SIZES 기준
±TABLE_BOX_SIZE_JITTER)에 더해 테이블 위 3개 박스의 위치(dx,dy)도 매 실행마다
무작위로 흔든다.

[38.py 대비 달라진 점]
- 물리와 무관한 카메라 준비(build_mobile_manipulator/find_camera_prim_path/
  Camera 초기화)를 박스 스폰보다 먼저 하도록 재배치했다 - 실제 관절/그리퍼가
  박스 낙하 도중 씬에 존재하는 일은 여전히 없다(그 부분은 그대로 박스 스폰
  뒤에 있음), 카메라 프림만 조금 먼저 만들어서 그 실제 내부 파라미터
  (get_intrinsics_matrix)로 "지금 스캔하면 ROI 안에 확실히 들어오는 영역"을
  미리 계산할 수 있게 한 것뿐이다.
- 이 계산된 영역(SAFE_PLACEMENT_RECT) 안에서, 박스끼리 안 겹치고(RANSAC/DBSCAN이
  붙여서 오탐 안 내도록 최소 간격까지 확보) 테이블도 안 벗어나게 거부 샘플링으로
  각 박스의 (dx,dy)를 뽑는다. 그 외 로직(스캔/필터/저장)은 38.py와 동일 -
...
#   HEADLESS=1 isaac_python 39.crate_scan_setup_random_layout.py
...
def _jittered_size(base_size):
    return tuple(
        round(v * random.uniform(1.0 - TABLE_BOX_SIZE_JITTER, 1.0 + TABLE_BOX_SIZE_JITTER), 4)
        for v in base_size
    )
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"39.crate_scan_setup_random_layout.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_39scan_random_layout"
...
print("\n[완료] 39.crate_scan_setup_random_layout.py 끝.\n", flush=True)
