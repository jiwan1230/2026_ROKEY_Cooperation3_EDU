...
# 사용자 요청(100-1 재작성: 1차 시도(Large 1개+Medium+Small)는 사용자가 원한
...
# 가로지르지 않는 한) 99-1/100-1 1차 시도 때 겪은 "노출면이 둘로 쪼개지는"
# 문제도 생기지 않는다.
...
# 계산한다(뒤쪽 "카트 안 적층 박스 스폰+정착" 지점 참고 - step_hold()가 이미
# 정의돼 있어야 해서 실제 DynamicCuboid 생성은 거기서 진행한다).
CART_BOX_SPECS = [
...
# 재면 박스가 기울어졌을 때 신뢰할 수 없다) - 크기는 스폰 전에도 이미 알고 있으므로 미리 만든다.
...
# 99-1.cart_scan_dual_side_holonomic.py에도 동일하게 적용 - CART_BOX_SPECS
# 정의부의 설명 참고). 실측 확인: 부모(Large2)가 자식을 2개(Medium2+Small)나
# 한쪽 끝에 얹은 무거운 배치라 착지 충격에 밀리기 쉬웠고, 기존(전체 동시 낙하)
# 방식으로는 자식이 완전히 빗나갔다(Medium2가 Large2 옆 17cm, Small은 Large2
# 자체보다 낮은 높이까지 떨어짐) - 부모를 먼저 정착시키고 그 실측 위치를
# 자식의 스폰 좌표로 쓴다.
...
for _level_idx, _level in enumerate(_cart_box_levels):
    for _box_name, _box_size, (_dx, _dy), _mass_kg in _level:
        if _box_name in CART_STACK_BASE_NAMES:
            _parent_x, _parent_y = cart_center_xy[0], cart_center_xy[1]
            _spawn_z = CART_BASKET_FLOOR_Z + CART_BOX_DROP_HEIGHT_ABOVE_FLOOR
        else:
            _parent_name = _STACK_PARENT[_box_name]
            _parent_x, _parent_y = _cart_box_xy_by_name[_parent_name]
            _spawn_z = (
                _cart_box_top_z_by_name[_parent_name] + _box_size[2] / 2.0 + _CART_STACK_TOP_SPAWN_MARGIN_M
            )
        _abs_x, _abs_y = _parent_x + _dx, _parent_y + _dy
        _cart_box_size_by_name[_box_name] = _box_size
        ...
        cart_box_objects[_box_prim_path] = DynamicCuboid(
            ...
            position=np.array([_abs_x, _abs_y, _spawn_z]),
    ...
    for _box_name, _box_size, _off, _m in _level:
        _prim = stage.GetPrimAtPath(f"/World/Box_{_box_name}")
        _actual = np.array(_prim.GetAttribute("xformOp:translate").Get())
        _cart_box_xy_by_name[_box_name] = (float(_actual[0]), float(_actual[1]))
        _cart_box_top_z_by_name[_box_name] = float(_actual[2]) + _box_size[2] / 2.0
    print(
