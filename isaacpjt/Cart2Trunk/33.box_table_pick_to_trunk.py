...
# 기본은 헤드리스(자동 검증용) - 직접 눈으로 보고 싶으면 HEADLESS=0으로 실행:
#   HEADLESS=0 isaac_python 33.box_table_pick_to_trunk.py
...
PLACE_DIMENSIONS = (0.19209560751914978, 0.2802707701921463, 0.17039998807013035)  # 알고리즘이 계산한 박스 크기
TRUNK_FLOOR_WORLD_Z = 0.435  # 참고용 - 실제로는 안 씀(reach 문제로 못 내려감)
TRUNK_X_MIN = 3.11  # 17/28.cart_to_trunk_pick_place*.py와 동일 확립된 상수 - 트렁크 입구
...
PLACE_HOVER_Z = PLACE_RELEASE_WORLD_Z + 0.15  # 28.py의 APPROACH_HOVER 패턴

# 텔레포트 목표 계산: 9.py reach-sweep에서 검증된 로컬 오프셋만큼 떨어진 지점에 로봇을 놓는다.
# 처음엔 dx=0.35로 했더니 섀시가 차체 범퍼와 거의 겹쳐서(스크린샷으로 확인), 텔레포트 직후
# 팔이 이미 트렁크 안에 들어가 있는 것처럼 보여 "배치 과정"이 안 보인다는 사용자 지적 -
# dx를 늘려서 섀시가 차체와 눈에 띄게 떨어지게 하고, 대신 PLACE 자체를 아래에서
# 입구(wp_entrance)->내부(wp_interior)->하강 다단계로 만들어서 팔이 실제로 뻗어
# 들어가는 과정이 보이게 한다.
REACH_DX = 0.45
...
# 박스 스캔 결과 + 적재 알고리즘이 실제로 계산해 낸 목표 위치/크기를, 어떤 pick&place 동작도
# 하기 전에 마커로 시각화해서 따로 저장한다 - 나중에 실제 결과와 비교하거나 남에게 설명할 때
# "알고리즘이 원래 원했던 자리"를 보여주는 참고 자료로 쓴다. TRUNK_FLOOR_WORLD_Z(0.435)는
# reach 문제로 실제로는 못 내려가는 "진짜" 목표 높이 - 그 타협 이전의 순수 알고리즘 출력이다.
target_marker = VisualCuboid(
    prim_path="/World/PlaceTargetPreview",
    name="place_target_preview",
    position=np.array([PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], TRUNK_FLOOR_WORLD_Z + PLACE_DIMENSIONS[2] / 2.0]),
...
snapshot(
    world, viewport,
    eye=[CHASSIS_TARGET_XY[0] - 1.2, CHASSIS_TARGET_XY[1] - 1.5, 1.6],
    target=[PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], TRUNK_FLOOR_WORLD_Z],
    fname="_pick_to_trunk_00b_target_preview.png",
)
target_marker.set_visibility(False)  # 이후 진행에 방해되지 않게 숨김 (삭제는 안 함)
...
# ================= 3. PLACE (입구->내부->하강, 28.py의 wp_entrance/interior 패턴) =================
# 텔레포트 직후 팔이 (이전 자세가 그대로 강체 이동돼서) 이미 트렁크 근처에 겹쳐 보이는 문제를
# 피하려고, 곧장 목표로 가지 않고 "입구 -> 내부 -> 하강" 순서로 눈에 보이게 뻗어간다.
# 처음엔 "차 밖(자기 베이스 바로 위, dx=0)"도 웨이포인트로 넣었는데, 이건 9.py reach-sweep이
# 검증한 범위(dx 0.25~0.45m)를 완전히 벗어난 지점이라 오히려 크게 못 미쳤다(err 0.27m, 실측) -
# 검증 범위 안인 wp_entrance(dx=0.246)부터 시작하도록 그 웨이포인트는 뺐다.
wp_entrance = (TRUNK_X_MIN, PLACE_WORLD_XY[1], PLACE_HOVER_Z)
wp_interior = (PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], PLACE_HOVER_Z)
place_release_pos = (PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], PLACE_RELEASE_WORLD_Z)

move_link6(world, controller, robot, wp_entrance, steps=300, hold_gripper_closed=True, label="place_entrance")
snapshot(
    world, viewport,
    eye=[chassis_target_pos[0] - 1.2, chassis_target_pos[1] - 1.5, 1.6],
    target=[PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], TRUNK_FLOOR_WORLD_Z],
    fname="_pick_to_trunk_03_approaching.png",
)
move_link6(world, controller, robot, wp_interior, steps=300, hold_gripper_closed=True, label="place_interior")
...
# 같은 웨이포인트를 반대 순서로 지나며 빠져나온다 (28.py의 retract 패턴).
move_link6(world, controller, robot, wp_interior, steps=200, label="place_retract_interior")
move_link6(world, controller, robot, wp_entrance, steps=200, label="place_retract_entrance")

snapshot(
    world, viewport,
    eye=[chassis_target_pos[0] - 1.2, chassis_target_pos[1] - 1.5, 1.6],
    target=[PLACE_WORLD_XY[0], PLACE_WORLD_XY[1], TRUNK_FLOOR_WORLD_Z],
    fname="_pick_to_trunk_04_placed.png",
)
