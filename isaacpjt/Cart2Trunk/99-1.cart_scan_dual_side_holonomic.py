...
# 사용자 요청(99-1 재작성: 1차 시도(Large 1개+Medium+Small)는 사용자가 원한
# 구조가 아니었음 - 실제로 원한 건 "Large 2개, Large1 위엔 Medium 1개만,
# Large2 위엔 Medium 1개+Small 1개가 나란히") - Medium이 두 벌 필요해서
# Medium1(Large1 위)/Medium2(Large2 위)로 이름을 나눈다. 99번/100번은 그대로
# 두고 이 파일에서만 반영한다. 트렁크 크기 제약(모든 변 0.21m 미만)과 "자식이
# 부모 가장자리 밖으로 삐져나오면 안 된다"는 조건은 그대로 유지한다.
...
# Large1 위 Medium1: 자식이 하나뿐이라 그냥 중앙(local offset 0,0)에 둬도 사방에
# 여유(약 5cm)가 남아 가장자리를 넘지 않는다 - 이 경우엔 Large1의 노출면이
# Medium1을 감싸는 "액자 테두리"처럼 사방으로 이어져 있어서(자식이 폭 전체를
# 가로지르지 않는 한) 99-1 1차 시도 때 겪은 "노출면이 둘로 쪼개지는" 문제도
# 생기지 않는다.
_medium1_local = (0.0, 0.0)

# Large2 위 Medium2+Small: 실측 확인(1차 시도 - dx=0으로 정중앙에 나란히 두면
# 그 x구간에서 Large의 폭 전체를 덮어버려 노출면이 둘로 쪼개져 검출됨, 같은
# 높이에서 후보 2개로 갈라짐) - 폭(y)축으로 최소 간격(_CHILD_GAP_M)을 두고
# 나란히 놓되, 길이(x)축 한쪽 끝으로 같이 밀어서 반대쪽에 Large2의 노출면이
# "하나로 이어진" 채 남게 한다.
_CHILD_GAP_M = 0.01
_children_total_y = MEDIUM_SIZE[1] + _CHILD_GAP_M + SMALL_SIZE[1]
assert _children_total_y <= LARGE_SIZE[1], (
    f"Medium+Small 폭 합({_children_total_y:.3f})이 Large 폭({LARGE_SIZE[1]:.3f})보다 커서 "
    "가장자리 밖으로 삐져나옵니다 - LARGE_SIZE를 키우거나 _CHILD_GAP_M을 줄이세요."
)
_medium2_dy = -(_children_total_y / 2.0 - MEDIUM_SIZE[1] / 2.0)
_small_dy = (_children_total_y / 2.0 - SMALL_SIZE[1] / 2.0)
_children_dx = LARGE_SIZE[0] / 2.0 - max(MEDIUM_SIZE[0], SMALL_SIZE[0]) / 2.0 - _EDGE_SAFETY_MARGIN_M

CART_STACK_BASE_NAMES = ["Large1", "Large2"]
_STACK_PARENT = {"Medium1": "Large1", "Medium2": "Large2", "Small": "Large2"}
# 자식(Medium1/Medium2/Small)의 offset은 여기서부터는 "카트 중심" 기준이 아니라
# "부모(Large1/Large2) 중심" 기준 로컬 offset이다 - 88.cart_scan_holonomic.py와
# 동일하게, 실제 스폰은 부모가 정착한 뒤 그 실측 위치에 이 로컬 offset을 더해서
# 계산한다(바로 아래 단계별 스폰 루프 참고).
CART_BOX_SPECS = [
    ...
    ("Medium1", MEDIUM_SIZE, _medium1_local, 0.6),
    ("Medium2", MEDIUM_SIZE, (_children_dx, _medium2_dy), 0.6),
    ("Small", SMALL_SIZE, (_children_dx, _small_dy), 0.3),
]
...
# 위 자식(Medium1/Medium2/Small) - 이 시나리오는 2단이라 레벨이 항상 이 둘뿐이다.
