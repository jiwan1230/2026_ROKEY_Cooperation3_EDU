"""
42.crate_scan_setup_table_box_verify.py
41.crate_scan_setup_random_obstacles.py의 복사본 - 35/38/39/41.py는 그대로 두고
(전부 잘 동작하는 버전이라 보존), 여기서는 두 가지를 추가한다:

1. 더미 장애물처럼(41.py) 테이블 박스도 "이번에 실제로 샘플링한 위치/크기" vs
   "박스 스캔이 실제로 검출한 위치/크기"를 박스별로 직접 대조하는 로그를 추가했다
   - 지금까지는 크레이트 장애물만 이 비교가 있었고 테이블 박스는 없었음. Vision
   검출 정확도를 "다양한 랜덤 배치 상황"에서 정량적으로 판단하기 위한 것.
2. 테이블 스캔 시 별도 프로세스로 띄우는 검출기를 기존 run_scan_once.py 대신
   perception/run_scan_once_accumulate.py로 바꿀 수 있다 - "여러 프레임 각각
   검출 후 개수로 투표" 대신 "여러 프레임의 raw point cloud를 먼저 합친 뒤 딱
   한 번만 검출"하는 실험(카메라를 안 움직이는 저비용 버전 - 42.py 자체의 스캔
   흐름/마커 핸드셰이크는 전혀 안 바뀐다, 터미널 B에서 돌리는 스크립트만 다름).
...
#   HEADLESS=1 isaac_python 42.crate_scan_setup_table_box_verify.py
...
_placement_order = ["Large", "Medium", "Small"]
_sampled_offsets = {}
_placed_rects = []
...
def converge_to_pose(controller, eye, look_at, label, cur_base_pos, cur_base_quat, cur_R_base):
    ...
    transform_payload = {
        ...
        "note": f"42.crate_scan_setup_table_box_verify.py의 '{label}' 스캔 자세 전용. 팔이 이 자세를 벗어나면 무효.",
...
# [42.py 검증] 41.py가 크레이트 더미에 이미 했던 것과 같은 방식 - 이번 실행에서 실제로
# 샘플링한(스폰에 쓴) 테이블 박스 위치/크기와, 방금 스캔이 검출해낸 결과를 박스별로
# 직접 대조한다. 랜덤 배치를 다양하게 돌려보면서 Vision 검출 정확도를 정량적으로
# 판단하기 위한 것(사용자 요청) - corners_m은 base_link 로컬 좌표라 world로 변환해서
# 비교한다(더미 검증과 동일한 변환식).
print("\n[검증: 테이블 박스 샘플링 vs 검출] --------------------------------------", flush=True)
for _tname in ("Small", "Medium", "Large"):
    _tdx, _tdy = _sampled_offsets[_tname]
    _tgx, _tgy = CART_POS[0] + _tdx, CART_POS[1] + _tdy
    _tgw, _tgd, _tgh = TABLE_BOX_SIZES[_tname]
    _best_b, _best_dist = None, None
    for _b in table_real_boxes:
        _pts_world = np.asarray(_b["corners_m"]) @ R_base.T + base_pos
        _bc = _pts_world.mean(axis=0)
        _dist = ((_bc[0] - _tgx) ** 2 + (_bc[1] - _tgy) ** 2) ** 0.5
        if _best_dist is None or _dist < _best_dist:
            _best_b, _best_dist = _b, _dist
    if _best_b is None:
        print(f"[검증] {_tname}: 샘플링 center=({_tgx:.3f},{_tgy:.3f}) size=({_tgw:.3f}x{_tgd:.3f}x{_tgh:.3f}) "
    ...
    _dims = _mx - _mn
    print(f"[검증] {_tname}: 샘플링 center=({_tgx:.3f},{_tgy:.3f}) size=({_tgw:.3f}x{_tgd:.3f}x{_tgh:.3f}) "
          f"vs 검출 center=({(_mn[0]+_mx[0])/2:.3f},{(_mn[1]+_mx[1])/2:.3f}) "
          f"size=({_dims[0]:.3f}x{_dims[1]:.3f}x{_dims[2]:.3f}) center오차={_best_dist:.3f}m", flush=True)
print("----------------------------------------------------------------------\n", flush=True)
...
_archive_dir = RESULTS_DIR / "runs" / f"{_run_stamp}_42scan_table_box_verify"
...
print("\n[완료] 42.crate_scan_setup_table_box_verify.py 끝.\n", flush=True)
