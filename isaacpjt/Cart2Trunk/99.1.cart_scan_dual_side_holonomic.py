"""
99.1.cart_scan_dual_side_holonomic.py

99.cart_scan_dual_side_holonomic.py의 복사본(사용자 지시, 2026-07-26) - 99번은 아직
건드리지 않는다. 88.cart_scan_holonomic.py가 그 이후 실측으로 발전시킨 "azimuth
대각선 시점" 다중 스캔(88번 본문 CART_SCAN_VIEWPOINTS/CART_SCAN_AZIMUTH_DIAGONAL_*
참고 - 정면 5곳뿐 아니라 일부 x_offset에서 좌우로 ±20/35도 돌아본 시점을 추가해서
패럴랙스를 늘리는 방식, M1/M2처럼 서로 다른 위치의 자식 박스를 다른 각도에서 봐야
가장자리 너머까지 "돌아서 보는" 효과가 난다는 게 실측으로 확인됨)을, 99번의 축
규칙(88번은 standoff=X/strafe=Y, 99번은 카트 90도 회전으로 standoff=Y/strafe=X - 아래
스캔 루프 섹션에서 축을 맞바꾸고 부호를 조정)에 맞춰 이식한다. 그 외 로직(박스 배치,
드라이브 안전 패턴, ROI 필터 범위 등)은 99번과 동일하게 유지한다.

[이하 원본 99.py 헤더, 그대로 유지]
----
...
# (99.1: horizontal_offset은 이제 스캔 루프 안에서 시점마다(tilt_deg 기준으로)
# 다시 계산한다 - azimuth 대각선 시점도 같은 공식을 쓰므로 모듈 전역 상수 대신
# 루프 지역 변수 horizontal_offset_i로 옮겼다. 아래 스캔 루프 참고.)
...
# ================= 2. 다중 시점 스캔 (베이스 strafe + azimuth 대각선으로 시점 다양화) =================
...
#
# [88.py에서 이식 - 사용자 지시, 2026-07-26] 정면(azimuth=0) strafe만으로는 다중
# 시점의 한계가 있다는 게 88번에서 실측으로 확인됐다: 카메라가 서 있는 좌우 방향
# (방위각)이 매 시점 항상 동일해서, 부모 박스의 "깨끗한 노출 띠"가 좁을 때(자식이
# 거의 가장자리까지 덮은 경우) 그 너머를 "돌아서 보는" 패럴랙스 효과가 전혀 안
# 생겼다. azimuth=0 baseline(기존 5개 strafe 시점, 검증된 그대로 유지)에 더해,
# 일부 strafe 위치에서 eye를 좌우 대각선(azimuth ±20/35도)으로 돌려본 시점을
# 추가한다 - look_at은 그대로 두고 eye만 원뿔 모양으로 움직이므로 도달 거리(reach)는
# baseline과 동일해서 IK 문제 재발 위험이 tilt 자체를 바꾸는 것보다 낮다(88번
# 실측 확인).
#
# 축 배정이 88번과 반대다(위 eye/look_at 주석과 동일한 이유) - 88번은 standoff가
# X라 cos(azimuth) 성분을 eye의 X(standoff축)에, sin(azimuth) 성분을 eye의 Y
# (strafe축, look_at과 동일한 strafe_y에 직접 더함)에 넣었다. 99번은 standoff가
# Y이므로 cos 성분은 eye의 Y에(부호는 마이너스 - 로봇이 -Y쪽에서 접근하므로),
# sin 성분은 eye의 X(strafe축, look_at과 동일한 strafe_x에 직접 더함)에 넣는다.
# azimuth=0이면 sin=0/cos=1이 되어 기존 baseline 공식과 완전히 동일해진다.
#
...
# 88.py와 동일(값 그대로 이식) - 이 strafe 위치들에서만 azimuth 대각선 시점을
# 추가로 찍는다(baseline 5곳은 그대로 유지, 여기 뽑힌 곳만 대각선이 추가됨).
# 88번은 M1/M2가 있던 [0.0, 0.14] 부근을 골랐던 것 - 99번에서도 카트 중앙 부근에
# Medium/Small이 있으므로(CART_BOX_SPECS 오프셋 참고) 동일한 값을 기본으로 둔다.
CART_SCAN_AZIMUTH_DIAGONAL_X_OFFSETS = [0.0, 0.14]
CART_SCAN_AZIMUTH_DIAGONAL_DEG = [-35.0, -20.0, 20.0, 35.0]
CART_SCAN_VIEWPOINTS = (
    [(SCAN_TILT_FROM_VERTICAL_DEG, x_offset, 0.0) for x_offset in CART_SCAN_STRAFE_X_OFFSETS]
    + [
        (SCAN_TILT_FROM_VERTICAL_DEG, x_offset, az_deg)
        for x_offset in CART_SCAN_AZIMUTH_DIAGONAL_X_OFFSETS
...
for i, (tilt_deg, x_offset, azimuth_deg) in enumerate(CART_SCAN_VIEWPOINTS):
    strafe_x = cart_center_xy[0] + x_offset
    drive_to(
        target_x=strafe_x, target_y=SCAN_STANDOFF_XY[1], target_yaw_deg=0.0,
        label=f"스캔 위치 {i}(tilt={tilt_deg:.0f}deg,x_offset={x_offset:+.2f},az={azimuth_deg:+.0f}deg)",
    )
    ...
    # 기준 상대 자세"로 보면 거의 동일하다 - look_at이 strafe_x를 그대로 따라가는
    ...
    # 처음부터 다시 충분한 스텝을 준다(88.py의 is_new_pose_family 판정 그대로).
    #
    # eye/look_at의 축 배정이 88번과 반대다 - 88번은 standoff가 X(폭)라 eye의 X에
    # cos(azimuth) 틸트 오프셋을, Y(길이, strafe축)에 strafe_y+sin(azimuth) 오프셋을
    # 넣었다. 이제 standoff가 Y(폭)이므로 cos 오프셋은 eye의 Y에(로봇이
    # SCAN_STANDOFF_XY처럼 -Y쪽에서 접근하므로 부호도 마이너스 - eye가 카트
    # 중심보다 로봇 쪽(-Y)으로 더 물러나 있어야 35.py와 동일한 기하학적 틸트가
    # 나온다), sin 오프셋은 eye의 X(strafe축, strafe_x에 직접 더함)에 들어간다.
    # azimuth=0이면 sin=0/cos=1이라 기존 baseline 공식과 완전히 같아진다.
    horizontal_offset_i = EYE_HEIGHT_ABOVE_CART * np.tan(np.radians(tilt_deg))
    azimuth_rad_i = np.radians(azimuth_deg)
    scan_eye_i = np.array([
        strafe_x + horizontal_offset_i * np.sin(azimuth_rad_i),
        cart_center_xy[1] - horizontal_offset_i * np.cos(azimuth_rad_i),
    ...
    print(f"[카트 스캔 {i}] tilt={tilt_deg:.0f}deg x_offset={x_offset:+.2f} az={azimuth_deg:+.0f}deg "
