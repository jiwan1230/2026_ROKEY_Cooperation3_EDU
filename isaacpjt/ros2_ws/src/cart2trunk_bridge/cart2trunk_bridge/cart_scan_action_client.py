...
def main(args=None):
    ...
    total_chunks = result.ply_total_chunks
    missing = [i for i in range(total_chunks) if i not in chunks]
    if missing:
        _fail(f"청크 유실: {len(missing)}/{total_chunks}개 누락 (인덱스 예: {missing[:5]})")

    ply_data = b"".join(chunks[i] for i in range(total_chunks))
    if len(ply_data) != result.ply_total_bytes:
        _fail(f"수신 바이트 수 불일치: 기대={result.ply_total_bytes} 실제={len(ply_data)}")
